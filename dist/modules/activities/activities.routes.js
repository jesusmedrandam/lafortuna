...
async function replaceAnimals(client, activityId, ids) {
    const unique = [...new Set(ids)];
    const available = await client.query(`SELECT id_animal FROM animal WHERE id_animal=ANY($1::uuid[]) AND estado='ACTIVO' AND deleted_at IS NULL`, [unique]);
    if (available.rowCount !== unique.length)
        throw new ValidationError('Uno o más animales no están activos o disponibles.');
    await client.query('UPDATE actividad_animal SET deleted_at=NOW() WHERE id_actividad=$1 AND deleted_at IS NULL', [activityId]);
    for (const id of unique)
        await client.query(buildInsert('actividad_animal', { id_actividad: activityId, id_animal: id }));
}
async function resolveAppliedMark(client, typeId, markId) {
    const type = (await client.query('SELECT codigo FROM tipo_actividad WHERE id_tipo_actividad=$1 AND activo=TRUE AND deleted_at IS NULL', [typeId])).rows[0];
    if (!type)
        throw new ValidationError('El tipo de actividad no está disponible.');
    if (type.codigo !== 'HERRAJE')
        return null;
    if (!markId)
        throw new ValidationError('Seleccione el fierro aplicado durante el herraje.');
    const mark = await client.query('SELECT 1 FROM marquilla WHERE id_marquilla=$1 AND activo=TRUE AND deleted_at IS NULL', [markId]);
    if (!mark.rowCount)
...
async function applyCurrentMark(client, ids, markId) {
    if (!markId)
        return;
    await client.query(`UPDATE animal SET id_marquilla=$2,updated_at=NOW()
     WHERE id_animal=ANY($1::uuid[]) AND deleted_at IS NULL`, [[...new Set(ids)], markId]);
}
export const activitiesRouter = Router();
activitiesRouter.get('/', requirePermission('ACTIVIDAD_CONSULTAR'), asyncHandler(async (_req, res) => ok(res, (await pool.query(`SELECT ac.*,ta.nombre tipo_actividad,ta.codigo tipo_actividad_codigo,
   ...
   WHERE ac.deleted_at IS NULL ORDER BY ac.fecha DESC,ac.created_at DESC`)).rows)));
