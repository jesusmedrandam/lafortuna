...
function payload(body, partial = false) {
    ...
    if (typeof body === 'object' && body !== null && 'data' in body) {
        try {
            value = JSON.parse(String(body.data));
        }
        catch {
...
async function replaceUsers(client, markId, userIds, registeredBy) {
    const uniqueIds = [...new Set(userIds)];
    const found = await client.query(`SELECT id_usuario FROM usuario
     WHERE id_usuario=ANY($1::uuid[]) AND deleted_at IS NULL AND activo=TRUE`, [uniqueIds]);
    if (found.rowCount !== uniqueIds.length)
        throw new ValidationError('Uno o más usuarios seleccionados no están disponibles.');
...
marksRouter.get('/usuarios', requirePermission('CATALOGO_CONSULTAR', 'ANIMAL_CONSULTAR'), asyncHandler(async (_req, res) => ok(res, (await pool.query(`SELECT id_usuario,TRIM(CONCAT(nombres,' ',apellidos)) nombre,correo
   FROM usuario WHERE deleted_at IS NULL AND activo=TRUE AND correo_verificado=TRUE ORDER BY nombres,apellidos`)).rows)));
marksRouter.get('/', requirePermission('CATALOGO_CONSULTAR', 'ANIMAL_CONSULTAR'), asyncHandler(async (_req, res) => ok(res, (await pool.query(`SELECT m.*,
   ...
   FROM marquilla m WHERE m.deleted_at IS NULL ORDER BY m.activo DESC,m.nombre,m.codigo`)).rows)));
...
marksRouter.patch('/:id', requirePermission('CATALOGO_ADMINISTRAR'), upload.single('foto'), asyncHandler(async (req, res) => {
    ...
    if (!current)
        throw new NotFoundError('Fierro no encontrado.');
    const data = payload(req.body, true);
    const cloud = req.file ? await uploadMarkImage(req.file.buffer, id) : null;
    try {
        const row = await transaction(async (client) => {
            const { id_usuarios, ...mark } = data;
...
marksRouter.delete('/:id', requirePermission('CATALOGO_ADMINISTRAR'), asyncHandler(async (req, res) => {
    const result = await pool.query('UPDATE marquilla SET activo=FALSE,updated_at=NOW() WHERE id_marquilla=$1 AND deleted_at IS NULL', [routeParam(req.params.id, 'id')]);
    if (!result.rowCount)
        throw new NotFoundError('Fierro no encontrado.');
