...
function medicineRouteIds(body, required) {
    if (!Object.prototype.hasOwnProperty.call(body, 'id_vias_administracion')) {
        if (required)
            throw new ValidationError('Selecciona al menos una vía de administración.');
        return null;
    }
    return [...new Set(medicineRoutesSchema.parse(body.id_vias_administracion))];
}
async function replaceMedicineRoutes(client, medicineId, routeIds) {
    const valid = (await client.query(`SELECT id_via_administracion FROM via_administracion
     WHERE id_via_administracion=ANY($1::uuid[]) AND deleted_at IS NULL AND activo=TRUE`, [routeIds])).rows.map((row) => row.id_via_administracion);
    if (valid.length !== routeIds.length)
        throw new ValidationError('Una de las vías seleccionadas no existe o está inactiva.');
    await client.query('DELETE FROM medicamento_via_administracion WHERE id_medicamento=$1', [medicineId]);
    for (const routeId of routeIds)
        await client.query('INSERT INTO medicamento_via_administracion(id_medicamento,id_via_administracion) VALUES($1,$2)', [medicineId, routeId]);
}
async function assertTreatmentType(client, treatmentTypeId) {
    if (typeof treatmentTypeId !== 'string')
        throw new ValidationError('Selecciona el tipo de tratamiento del medicamento.');
    const found = (await client.query('SELECT 1 FROM tipo_tratamiento WHERE id_tipo_tratamiento=$1 AND deleted_at IS NULL AND activo=TRUE', [treatmentTypeId])).rows[0];
    if (!found)
        throw new ValidationError('El tipo de tratamiento no existe o está inactivo.');
}
...
catalogsRouter.post('/:catalog', requirePermission('CATALOGO_ADMINISTRAR'), asyncHandler(async (req, res) => {
    ...
    if (!Object.keys(data).length)
        throw new ValidationError('No hay campos válidos.');
    if (name !== 'medicamentos')
        return created(res, (await pool.query(buildInsert(def.table, data))).rows[0]);
    ...
    if (name === 'medicamentos') {
        ...
        const row = await transaction(async (client) => {
            ...
            if (!saved)
                throw new NotFoundError();
            if (routeIds)
                await replaceMedicineRoutes(client, id, routeIds);
