...
recordsRouter.get('/producciones/vacas-activas', requirePermission('PRODUCCION_CONSULTAR'), asyncHandler(async (req, res) => {
    ...
    const turn = z.enum(['MANANA', 'TARDE', 'NOCHE', 'UNICO']).catch('UNICO').parse(req.query.turno);
    const excludeId = z.string().uuid().optional().catch(undefined).parse(req.query.excluir_id);
    return ok(res, (await pool.query(`WITH vacas AS MATERIALIZED (
     ...
     WHERE NOT EXISTS(
       SELECT 1 FROM produccion_leche existing
       WHERE existing.id_vaca=v.id_animal
         AND existing.fecha_produccion::date=$1::date
         AND existing.turno=$2
         AND existing.deleted_at IS NULL
         AND ($3::uuid IS NULL OR existing.id_produccion<>$3::uuid)
     )
     ORDER BY v.nombre,v.codigo_arete`, [date, turn, excludeId ?? null])).rows);
}));
