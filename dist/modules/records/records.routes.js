           ...
           FOR SHARE`, [fatherId])).rows[0];
            ...
            for (const item of crias) {
                ...
                if (item.animal.id_grupo_actual) {
                    const group = (await client.query(`SELECT id_especie,id_categoria_animal,activo FROM grupo WHERE id_grupo=$1 AND deleted_at IS NULL`, [item.animal.id_grupo_actual])).rows[0];
