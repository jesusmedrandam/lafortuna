 ...
 */
export async function emitNotification(client, event) {
    const id = randomUUID();
    ...
    if (!notificationId && event.claveDedupe) {
        notificationId = (await client.query('SELECT id_notificacion FROM notificacion WHERE clave_dedupe=$1', [event.claveDedupe])).rows[0]?.id_notificacion;
    }
    if (!notificationId)
        return null;
    await client.query({
