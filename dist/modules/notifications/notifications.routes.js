...
notificationsRouter.get('/', asyncHandler(async (req, res) => {
    ...
    if (unreadOnly)
        where.push('nu.leida_at IS NULL');
    if (input.categoria) {
...
notificationsRouter.get('/resumen', asyncHandler(async (req, res) => ok(res, (await pool.query(`SELECT
   ...
   FROM notificacion_usuario WHERE id_usuario=$1`, [req.user.id])).rows[0])));
...
notificationsRouter.get('/preferencias', asyncHandler(async (req, res) => {
    const stored = (await pool.query(`SELECT categoria,mostrar_en_buzon,enviar_push
...
notificationsRouter.put('/preferencias', asyncHandler(async (req, res) => {
    ...
    await transaction(async (client) => {
        ...
        ON CONFLICT(id_usuario,categoria) DO UPDATE SET
          ...
          enviar_push=EXCLUDED.enviar_push,updated_at=NOW()`, [req.user.id, item.categoria, item.mostrar_en_buzon, item.enviar_push]);
        }
    }, req.user.id);
...
notificationsRouter.post('/dispositivos', asyncHandler(async (req, res) => {
    ...
    RETURNING id_dispositivo,plataforma,nombre_dispositivo,version_app,activo`, [req.user.id, input.token, input.plataforma, input.nombre_dispositivo ?? null, input.version_app ?? null])).rows[0];
    return ok(res, row);
}));
notificationsRouter.get('/dispositivos', asyncHandler(async (req, res) => ok(res, (await pool.query(`SELECT id_dispositivo,plataforma,nombre_dispositivo,version_app,activo,ultimo_uso_at,created_at
   FROM notificacion_dispositivo
   WHERE id_usuario=$1
   ORDER BY activo DESC,ultimo_uso_at DESC`, [req.user.id])).rows)));
...
notificationsRouter.patch('/:id/leer', asyncHandler(async (req, res) => {
    ...
    RETURNING id_notificacion,leida_at`, [routeParam(req.params.id, 'id'), req.user.id])).rows[0] ?? null;
    return ok(res, row);
}));
