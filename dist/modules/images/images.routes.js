...
function animalIds(value, primary) {
    let parsed = value;
...
async function assertAnimals(client, ids) {
    if (!ids.length)
        throw new ValidationError('Relaciona el archivo con al menos un animal.');
    const result = await client.query('SELECT id_animal FROM animal WHERE id_animal=ANY($1::uuid[]) AND deleted_at IS NULL', [ids]);
    if (result.rowCount !== ids.length)
        throw new ValidationError('Uno o más animales seleccionados ya no están disponibles.');
}
async function replaceRelations(client, imageId, ids, userId) {
...
async function replaceTags(client, imageId, ids, userId) {
    if (ids.length) {
        const valid = await client.query('SELECT id_etiqueta FROM etiqueta_multimedia WHERE id_etiqueta=ANY($1::uuid[]) AND activo=TRUE AND deleted_at IS NULL', [ids]);
        if (valid.rowCount !== ids.length)
            throw new ValidationError('Una o más etiquetas ya no están disponibles.');
    }
    await client.query('UPDATE animal_imagen_etiqueta SET deleted_at=NOW() WHERE id_imagen=$1 AND deleted_at IS NULL', [imageId]);
    for (const id of ids)
        await client.query(buildInsert('animal_imagen_etiqueta', { id_imagen: imageId, id_etiqueta: id, registrado_por: userId }));
}
async function imageWithAnimals(client, imageId) {
    return (await client.query(`SELECT i.*,COALESCE((SELECT jsonb_agg(jsonb_build_object(
     ...
     FROM animal_imagen i WHERE i.id_imagen=$1 AND i.deleted_at IS NULL`, [imageId])).rows[0];
}
...
animalImagesRouter.get('/', requirePermission('IMAGEN_CONSULTAR'), asyncHandler(async (req, res) => {
    ...
    const rows = (await pool.query(`SELECT i.*,COALESCE((SELECT jsonb_agg(jsonb_build_object(
     ...
     )) ORDER BY i.es_perfil DESC,i.orden,i.fecha_toma DESC,i.created_at DESC`, [id])).rows;
    return ok(res, rows);
}));
animalImagesRouter.post('/', requirePermission('IMAGEN_ADMINISTRAR'), mediaUpload.any(), asyncHandler(async (req, res) => {
    const file = uploadedFile(req);
    if (!file)
        throw new ValidationError('Debes enviar el archivo en el campo archivo o imagen.');
    ...
    try {
        const row = await transaction(async (client) => {
            await assertAnimals(client, ids);
            if (profile)
                await client.query('UPDATE animal_imagen SET es_perfil=FALSE WHERE id_animal=$1 AND deleted_at IS NULL', [primary]);
...
imagesRouter.get('/multimedia', requirePermission('IMAGEN_CONSULTAR'), asyncHandler(async (req, res) => {
    ...
    const params = [];
    const where = ['TRUE'];
    const add = (condition, value) => { params.push(value); where.push(condition.replaceAll('?', `$${params.length}`)); };
    if (filters.categoria)
        add('m.categoria=?', filters.categoria);
    if (filters.tipo)
        add('m.tipo_archivo=?', filters.tipo);
    if (filters.perfil)
        add('m.es_perfil=?::boolean', filters.perfil === 'SI');
    if (filters.fecha_desde)
        add('m.fecha_toma>=?::date', filters.fecha_desde);
    if (filters.fecha_hasta)
        add('m.fecha_toma<=?::date', filters.fecha_hasta);
    if (filters.id_animal)
        add('?::uuid=ANY(m.animal_ids)', filters.id_animal);
    if (filters.id_grupo)
        add('m.id_grupo=?::uuid', filters.id_grupo);
    if (filters.id_ubicacion)
        add('m.id_ubicacion=?::uuid', filters.id_ubicacion);
    if (filters.id_ubicacion_origen)
        add('m.id_ubicacion_origen=?::uuid', filters.id_ubicacion_origen);
    if (filters.id_ubicacion_destino)
        add('m.id_ubicacion_destino=?::uuid', filters.id_ubicacion_destino);
    if (filters.id_tipo_actividad)
        add('m.id_tipo_actividad=?::uuid', filters.id_tipo_actividad);
    if (filters.lado)
        add('m.lado=?', filters.lado);
    if (filters.id_etiqueta)
        add(`EXISTS(
...
imagesRouter.get('/', requirePermission('IMAGEN_CONSULTAR'), asyncHandler(async (req, res) => {
    ...
    const add = (condition, value) => { params.push(value); where.push(condition.replaceAll('?', `$${params.length}`)); };
    if (filters.tipo)
        add('i.tipo_archivo=?', filters.tipo);
    if (filters.fecha_desde)
        add('i.fecha_toma>=?::date', filters.fecha_desde);
    if (filters.fecha_hasta)
        add('i.fecha_toma<=?::date', filters.fecha_hasta);
    if (filters.id_etiqueta)
        add('EXISTS(SELECT 1 FROM animal_imagen_etiqueta iet WHERE iet.id_imagen=i.id_imagen AND iet.id_etiqueta=? AND iet.deleted_at IS NULL)', filters.id_etiqueta);
    if (filters.q) {
        params.push(`%${filters.q}%`);
        where.push(`(COALESCE(i.descripcion,'') ILIKE $${params.length} OR COALESCE(i.nombre_original,'') ILIKE $${params.length} OR EXISTS(
    ...
    if (filters.id_animal) {
        ...
        animalConditions.push(`af.id_animal=$${params.length}`);
    }
    if (filters.id_grupo) {
        params.push(filters.id_grupo);
        animalConditions.push(`af.id_grupo_actual=$${params.length}`);
    }
    if (filters.id_ubicacion) {
        params.push(filters.id_ubicacion);
        animalConditions.push(`af.id_ubicacion_actual=$${params.length}`);
    }
    if (filters.sexo) {
        params.push(filters.sexo);
        animalConditions.push(`af.sexo=$${params.length}`);
    }
...
imagesRouter.patch('/:id/perfil', requirePermission('IMAGEN_ADMINISTRAR'), asyncHandler(async (req, res) => {
    const row = await transaction(async (client) => {
        ...
        if (found.rows[0].tipo_archivo === 'VIDEO')
            throw new ValidationError('Un video no puede utilizarse como foto de perfil.');
...
imagesRouter.patch('/:id', requirePermission('IMAGEN_ADMINISTRAR'), asyncHandler(async (req, res) => {
    ...
    const row = await transaction(async (client) => {
        ...
        if (Object.prototype.hasOwnProperty.call(req.body, 'id_animales')) {
            if (found.es_perfil)
                throw new ValidationError('La foto de perfil solo puede pertenecer a su animal principal.');
            await replaceRelations(client, id, animalIds(req.body.id_animales), req.user.id);
        }
        if (Object.prototype.hasOwnProperty.call(req.body, 'id_etiquetas'))
            await replaceTags(client, id, tagIds(req.body.id_etiquetas), req.user.id);
