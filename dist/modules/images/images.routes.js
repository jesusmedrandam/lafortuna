...
animalImagesRouter.get('/', requirePermission('IMAGEN_CONSULTAR'), asyncHandler(async (req, res) => {
    ...
    const rows = (await pool.query(`SELECT i.*,COALESCE((SELECT jsonb_agg(jsonb_build_object(
     ...
     )) ORDER BY i.es_perfil DESC,i.orden,i.created_at DESC`, [id])).rows;
    return ok(res, rows);
}));
...
imagesRouter.get('/', requirePermission('IMAGEN_CONSULTAR'), asyncHandler(async (req, res) => {
    ...
    if (filters.fecha_desde)
        add('i.created_at>=?::date', filters.fecha_desde);
    if (filters.fecha_hasta)
        add("i.created_at<?::date + INTERVAL '1 day'", filters.fecha_hasta);
    if (filters.q) {
...
imagesRouter.patch('/:id', requirePermission('IMAGEN_ADMINISTRAR'), asyncHandler(async (req, res) => {
    ...
    const row = await transaction(async (client) => {
        ...
        await client.query('UPDATE animal_imagen SET descripcion=COALESCE($2,descripcion),orden=COALESCE($3,orden) WHERE id_imagen=$1', [id, req.body.descripcion, req.body.orden]);
