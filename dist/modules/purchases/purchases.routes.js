...
async function purchaseType(client, id) {
    const row = (await client.query(`SELECT id_tipo_producto_compra,nombre,es_animal FROM tipo_producto_compra
     WHERE id_tipo_producto_compra=$1 AND activo=TRUE AND deleted_at IS NULL`, [id])).rows[0];
    if (!row)
        throw new ValidationError('El tipo de producto seleccionado no está disponible.');
    return row;
}
async function validateInitialLocation(client, animal) {
    if (animal.id_grupo_actual) {
        const group = (await client.query(`SELECT id_categoria_animal,id_ubicacion_actual,activo FROM grupo
       WHERE id_grupo=$1 AND deleted_at IS NULL`, [animal.id_grupo_actual])).rows[0];
        if (!group?.activo)
            throw new ValidationError('El grupo inicial no está disponible.');
        if (group.id_categoria_animal !== animal.id_categoria_animal)
            throw new ValidationError('El grupo no corresponde a la categoría del animal.');
        if (group.id_ubicacion_actual !== (animal.id_ubicacion_actual ?? null))
            throw new ValidationError('La ubicación inicial debe ser la misma del grupo seleccionado.');
    }
    if (animal.id_ubicacion_actual) {
        const location = (await client.query(`SELECT id_categoria_animal,activo FROM ubicacion WHERE id_ubicacion=$1 AND deleted_at IS NULL`, [animal.id_ubicacion_actual])).rows[0];
        if (!location?.activo)
            throw new ValidationError('La ubicación inicial no está disponible.');
        if (location.id_categoria_animal !== animal.id_categoria_animal)
            throw new ValidationError('La ubicación no corresponde a la categoría del animal.');
    }
}
async function purchasePropertyId(client, animal) {
    if (animal?.id_grupo_actual) {
        const row = (await client.query('SELECT id_propiedad FROM grupo WHERE id_grupo=$1 AND deleted_at IS NULL', [animal.id_grupo_actual])).rows[0];
        if (row)
            return row.id_propiedad;
    }
    if (animal?.id_ubicacion_actual) {
        const row = (await client.query('SELECT id_propiedad FROM ubicacion WHERE id_ubicacion=$1 AND deleted_at IS NULL', [animal.id_ubicacion_actual])).rows[0];
        if (row)
            return row.id_propiedad;
    }
...
purchasesRouter.get('/', requirePermission('COMPRA_CONSULTAR'), asyncHandler(async (_req, res) => ok(res, (await pool.query(`SELECT c.*,t.nombre tipo_producto,t.codigo tipo_producto_codigo,t.es_animal,
   ...
   WHERE c.deleted_at IS NULL ORDER BY c.fecha_compra DESC,c.created_at DESC`)).rows)));
purchasesRouter.post('/', requirePermission('COMPRA_ADMINISTRAR'), asyncHandler(async (req, res) => {
    ...
    try {
        const row = await transaction(async (client) => {
            ...
            if (type.es_animal && !input.animal)
                throw new ValidationError('Completa los datos del animal comprado.');
            if (!type.es_animal && input.animal)
                throw new ValidationError('Este tipo de compra no debe crear un animal.');
            if (!type.es_animal && !input.producto)
                throw new ValidationError('Indica el producto comprado.');
        ...
        const db = error;
        if (db.code === '23505')
            throw new ConflictError('El arete o identificador del animal ya está registrado.');
        throw error;
    }
}));
purchasesRouter.patch('/:id', requirePermission('COMPRA_ADMINISTRAR'), asyncHandler(async (req, res) => {
    ...
    const row = await transaction(async (client) => {
        ...
        const type = await purchaseType(client, input.id_tipo_producto_compra);
        if (Boolean(found.id_animal) !== type.es_animal)
            throw new ValidationError('No se puede cambiar una compra de animal a producto ni viceversa.');
        if (!type.es_animal && !input.producto)
            throw new ValidationError('Indica el producto comprado.');
        const quantity = type.es_animal ? 1 : input.cantidad;
        const total = Number((quantity * input.valor_unitario).toFixed(2));
        return (await client.query(`UPDATE compra SET id_tipo_producto_compra=$2,fecha_compra=$3,proveedor=$4,producto=$5,
       cantidad=$6,id_unidad=$7,valor_unitario=$8,valor_total=$9,moneda=$10,observaciones=$11,updated_at=NOW()
       WHERE id_compra=$1 RETURNING *`, [id, input.id_tipo_producto_compra, input.fecha_compra, input.proveedor, input.producto ?? null, quantity,
            type.es_animal ? null : (input.id_unidad ?? null), input.valor_unitario, total, input.moneda.toUpperCase(), input.observaciones ?? null])).rows[0];
    }, req.user.id);
    cache.forgetModuleVersion('compras');
    return ok(res, row);
}));
