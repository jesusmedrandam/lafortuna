...
async function productiveLocation(database, input) {
    ...
    const property = (await database.query(`SELECT p.id_propiedad,p.es_principal,
     ...
     LIMIT 1`, selectedPropertyId ? [selectedPropertyId] : [])).rows[0];
    if (!property)
...
locationsRouter.patch('/:id', requirePermission('UBICACION_ADMINISTRAR'), asyncHandler(async (req, res) => {
    ...
    const row = await transaction(async (client) => {
        const current = (await client.query(`SELECT u.id_propiedad FROM ubicacion u
       ...
       WHERE u.id_ubicacion=$1 AND u.tipo='OTRO' AND u.deleted_at IS NULL
         AND p.es_principal=FALSE AND p.deleted_at IS NULL`, [id])).rows[0];
        if (!current)
        ...
        if (data.activo === false) {
          ...
          EXISTS(SELECT 1 FROM animal a JOIN ubicacion u ON u.id_ubicacion=a.id_ubicacion_actual
            WHERE u.id_propiedad=$1 AND a.estado='ACTIVO' AND a.deleted_at IS NULL) has_animals`, [current.id_propiedad])).rows[0];
        ...
        if (Object.keys(propertyData).length)
            await client.query(buildUpdate('propiedad_ganadera', 'id_propiedad', current.id_propiedad, propertyData));
...
locationsRouter.delete('/:id', requirePermission('UBICACION_ADMINISTRAR'), asyncHandler(async (req, res) => { const id = routeParam(req.params.id, 'id'); await transaction(async (client) => { const property = (await client.query(`SELECT id_propiedad FROM propiedad_ganadera WHERE id_propiedad=$1 AND es_principal=FALSE AND deleted_at IS NULL`, [id])).rows[0]; if (!property)
    throw new NotFoundError(); const usage = (await client.query(`SELECT EXISTS(SELECT 1 FROM ubicacion WHERE id_propiedad=$1 AND id_ubicacion<>$1 AND deleted_at IS NULL) has_locations,EXISTS(SELECT 1 FROM grupo WHERE id_propiedad=$1 AND activo=TRUE AND deleted_at IS NULL) has_groups,EXISTS(SELECT 1 FROM animal a JOIN ubicacion u ON u.id_ubicacion=a.id_ubicacion_actual WHERE u.id_propiedad=$1 AND a.estado='ACTIVO' AND a.deleted_at IS NULL) has_animals`, [id])).rows[0]; if (usage.has_locations || usage.has_groups || usage.has_animals)
    throw new ConflictError('No puede desactivar una propiedad que tenga potreros, corrales, grupos o animales activos.'); await client.query('UPDATE ubicacion SET deleted_at=NOW(),activo=FALSE WHERE id_propiedad=$1 AND deleted_at IS NULL', [id]); await client.query('UPDATE propiedad_ganadera SET deleted_at=NOW(),activa=FALSE WHERE id_propiedad=$1 AND deleted_at IS NULL', [id]); }, req.user.id); return noContent(res); }));
...
pasturesRouter.patch('/:id', requirePermission('POTRERO_ADMINISTRAR'), asyncHandler(async (req, res) => { const input = pastureSchema.partial().parse(req.body); const result = await transaction(async (c) => { const found = await c.query('SELECT p.id_ubicacion,u.id_propiedad FROM potrero p JOIN ubicacion u ON u.id_ubicacion=p.id_ubicacion WHERE p.id_potrero=$1 AND p.deleted_at IS NULL', [routeParam(req.params.id, 'id')]); if (!found.rows[0])
    throw new NotFoundError(); if (input.ubicacion) {
    const location = await productiveLocation(c, input.ubicacion);
    await ensurePropertyCanChange(c, found.rows[0].id_ubicacion, found.rows[0].id_propiedad, location.id_propiedad);
...
corralsRouter.patch('/:id', requirePermission('CORRAL_ADMINISTRAR'), asyncHandler(async (req, res) => { const input = corralSchema.partial().parse(req.body); const result = await transaction(async (c) => { const found = await c.query('SELECT c.id_ubicacion,u.id_propiedad FROM corral c JOIN ubicacion u ON u.id_ubicacion=c.id_ubicacion WHERE c.id_corral=$1 AND c.deleted_at IS NULL', [routeParam(req.params.id, 'id')]); if (!found.rows[0])
    throw new NotFoundError(); if (input.ubicacion) {
    const location = await productiveLocation(c, input.ubicacion);
    await ensurePropertyCanChange(c, found.rows[0].id_ubicacion, found.rows[0].id_propiedad, location.id_propiedad);
    await c.query(buildUpdate('ubicacion', 'id_ubicacion', found.rows[0].id_ubicacion, location));
