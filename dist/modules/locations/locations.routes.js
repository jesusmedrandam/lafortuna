...
async function productiveLocation(database, input) {
    if (input.id_propiedad_padre) {
        const property = (await database.query(`SELECT id_categoria_animal FROM ubicacion
       WHERE id_ubicacion=$1 AND tipo='OTRO' AND activo=TRUE AND deleted_at IS NULL`, [input.id_propiedad_padre])).rows[0];
        if (!property)
            throw new ValidationError('La propiedad externa seleccionada no está disponible.');
        return { ...input, id_categoria_animal: property.id_categoria_animal };
    }
    const owned = (await database.query(`SELECT id_categoria_animal FROM categoria_animal
     WHERE codigo='EN_PROPIEDAD' AND activo=TRUE AND deleted_at IS NULL LIMIT 1`)).rows[0];
    if (!owned)
        throw new ValidationError('No está configurada la categoría de animales en propiedad.');
    return { ...input, id_categoria_animal: owned.id_categoria_animal, id_propiedad_padre: null };
}
async function ensurePropertyCanChange(database, locationId, currentPropertyId, nextPropertyId) {
    if (nextPropertyId === undefined || nextPropertyId === currentPropertyId)
        return;
    const usage = (await database.query(`SELECT
       EXISTS(SELECT 1 FROM animal WHERE id_ubicacion_actual=$1 AND estado='ACTIVO' AND deleted_at IS NULL) has_animals,
       EXISTS(SELECT 1 FROM grupo WHERE id_ubicacion_actual=$1 AND activo=TRUE AND deleted_at IS NULL) has_groups`, [locationId])).rows[0];
...
} return ok(res, (await pool.query(`SELECT u.*,ca.nombre categoria,ca.codigo categoria_codigo,p.nombre propiedad,(SELECT COUNT(*)::int FROM animal a WHERE a.id_ubicacion_actual=u.id_ubicacion AND a.deleted_at IS NULL AND a.estado='ACTIVO') total_animales FROM ubicacion u JOIN categoria_animal ca ON ca.id_categoria_animal=u.id_categoria_animal LEFT JOIN ubicacion p ON p.id_ubicacion=u.id_propiedad_padre AND p.deleted_at IS NULL WHERE ${filters.join(' AND ')} ORDER BY u.activo DESC,ca.nombre,COALESCE(p.nombre,u.nombre),u.nombre`, params)).rows); }));
...
locationsRouter.delete('/:id', requirePermission('UBICACION_ADMINISTRAR'), asyncHandler(async (req, res) => { const id = routeParam(req.params.id, 'id'); const usage = (await pool.query(`SELECT EXISTS(SELECT 1 FROM ubicacion WHERE id_propiedad_padre=$1 AND deleted_at IS NULL) has_locations,EXISTS(SELECT 1 FROM grupo WHERE id_ubicacion_actual=$1 AND activo=TRUE AND deleted_at IS NULL) has_groups,EXISTS(SELECT 1 FROM animal WHERE id_ubicacion_actual=$1 AND estado='ACTIVO' AND deleted_at IS NULL) has_animals`, [id])).rows[0]; if (usage.has_locations || usage.has_groups || usage.has_animals)
    throw new ConflictError('No puede desactivar una propiedad que tenga potreros, corrales, grupos o animales activos.'); const r = await pool.query('UPDATE ubicacion SET deleted_at=NOW(),activo=FALSE WHERE id_ubicacion=$1 AND deleted_at IS NULL', [id]); if (!r.rowCount)
...
pasturesRouter.get('/', requirePermission('POTRERO_CONSULTAR'), asyncHandler(async (_req, res) => ok(res, (await pool.query(`
  WITH base AS (
    SELECT p.*,u.nombre,u.codigo,u.descripcion,u.activo,u.id_categoria_animal,u.id_propiedad_padre,
      propiedad.nombre propiedad,tu.nombre tipo_uso,um.simbolo unidad_area,
    ...
    GROUP BY p.id_potrero,u.id_ubicacion,propiedad.id_ubicacion,tu.nombre,um.simbolo
  )
  SELECT base.*,
...
pasturesRouter.get('/:id/resumen', requirePermission('POTRERO_CONSULTAR'), asyncHandler(async (req, res) => {
  ...
  `, [id])).rows[0];
    if (!pasture)
        throw new NotFoundError('Potrero no encontrado.');
...
pasturesRouter.patch('/:id', requirePermission('POTRERO_ADMINISTRAR'), asyncHandler(async (req, res) => { const input = pastureSchema.partial().parse(req.body); const result = await transaction(async (c) => { const found = await c.query('SELECT p.id_ubicacion,u.id_propiedad_padre FROM potrero p JOIN ubicacion u ON u.id_ubicacion=p.id_ubicacion WHERE p.id_potrero=$1 AND p.deleted_at IS NULL', [routeParam(req.params.id, 'id')]); if (!found.rows[0])
    throw new NotFoundError(); if (input.ubicacion) {
    await ensurePropertyCanChange(c, found.rows[0].id_ubicacion, found.rows[0].id_propiedad_padre, input.ubicacion.id_propiedad_padre);
    const location = await productiveLocation(c, input.ubicacion);
    await c.query(buildUpdate('ubicacion', 'id_ubicacion', found.rows[0].id_ubicacion, location));
...
corralsRouter.get('/', requirePermission('CORRAL_CONSULTAR'), asyncHandler(async (_req, res) => ok(res, (await pool.query(`SELECT c.*,u.nombre,u.codigo,u.descripcion,u.activo,u.id_categoria_animal,u.id_propiedad_padre,p.nombre propiedad,tc.nombre tipo_corral,(SELECT COUNT(*)::int FROM animal a WHERE a.id_ubicacion_actual=u.id_ubicacion AND a.estado='ACTIVO' AND a.deleted_at IS NULL) total_animales FROM corral c JOIN ubicacion u ON u.id_ubicacion=c.id_ubicacion LEFT JOIN ubicacion p ON p.id_ubicacion=u.id_propiedad_padre AND p.deleted_at IS NULL JOIN tipo_corral tc ON tc.id_tipo_corral=c.id_tipo_corral WHERE c.deleted_at IS NULL AND u.deleted_at IS NULL ORDER BY COALESCE(p.nombre,u.nombre),u.nombre`)).rows)));
...
corralsRouter.patch('/:id', requirePermission('CORRAL_ADMINISTRAR'), asyncHandler(async (req, res) => { const input = corralSchema.partial().parse(req.body); const result = await transaction(async (c) => { const found = await c.query('SELECT c.id_ubicacion,u.id_propiedad_padre FROM corral c JOIN ubicacion u ON u.id_ubicacion=c.id_ubicacion WHERE c.id_corral=$1 AND c.deleted_at IS NULL', [routeParam(req.params.id, 'id')]); if (!found.rows[0])
    throw new NotFoundError(); if (input.ubicacion) {
    await ensurePropertyCanChange(c, found.rows[0].id_ubicacion, found.rows[0].id_propiedad_padre, input.ubicacion.id_propiedad_padre);
    const location = await productiveLocation(c, input.ubicacion);
    await c.query(buildUpdate('ubicacion', 'id_ubicacion', found.rows[0].id_ubicacion, location));
