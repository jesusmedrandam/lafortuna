...
const reproductionConfigurationSchema = z.object({
    ...
    edad_vacona_meses: z.number().int().min(0).max(120),
    edad_torete_meses: z.number().int().min(0).max(120),
...
settingsRouter.get('/operaciones-animales', requirePermission('CATALOGO_CONSULTAR'), asyncHandler(async (_req, res) => {
    ...
    return ok(res, { propiedades: properties.rows, operaciones: animalOperationDefinitions, configuracion: configuration.rows });
}));
settingsRouter.put('/operaciones-animales', requirePermission('CATALOGO_ADMINISTRAR'), asyncHandler(async (req, res) => {
    ...
    await transaction(async (client) => {
        for (const item of input.configuracion) {
            await client.query(`INSERT INTO operacion_propiedad_animal(id_propiedad,codigo_operacion,permitido,registrado_por)
         VALUES($1,$2,$3,$4)
         ON CONFLICT(id_propiedad,codigo_operacion)
         DO UPDATE SET permitido=EXCLUDED.permitido,deleted_at=NULL,updated_at=NOW(),registrado_por=EXCLUDED.registrado_por`, [item.id_propiedad, item.codigo_operacion, item.permitido, req.user.id]);
        }
    }, req.user.id);
...
settingsRouter.get('/finca', requirePermission('CATALOGO_CONSULTAR'), asyncHandler(async (_req, res) => {
    const rows = (await pool.query(`SELECT
      ...
      COALESCE(cp.edad_vacona_meses,12)::int edad_vacona_meses,
      COALESCE(cp.edad_torete_meses,12)::int edad_torete_meses,
...
settingsRouter.put('/finca/:id/reglas-reproduccion', requirePermission('CATALOGO_ADMINISTRAR'), asyncHandler(async (req, res) => {
    ...
    const row = await transaction(async (client) => {
      ...
      edad_vacona_meses,edad_torete_meses,
    ...
    ON CONFLICT(id_propiedad) DO UPDATE SET
      ...
      edad_vacona_meses=EXCLUDED.edad_vacona_meses,
      edad_torete_meses=EXCLUDED.edad_torete_meses,
    ...
    RETURNING *`, [propertyId, input.dias_posparto_para_celo, input.dias_posparto_para_prenez,
            ...
            input.edad_minima_padre_meses, input.edad_vacona_meses, input.edad_torete_meses,
            input.permitir_segundo_celo, input.permitir_celo_falso_en_prenez,
