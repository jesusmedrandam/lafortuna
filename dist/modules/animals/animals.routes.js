...
const animalConditionSchema = z.string().trim().transform((value) => value.toUpperCase().replace(/\s+/g, '_')).pipe(z.string().min(1).max(40).regex(/^[A-Z0-9_]+$/, 'La condición del animal no es válida.'));
const schema = z.object({
    ...
    propietarios: z.array(ownerRelation).default([]).superRefine((items, ctx) => {
        ...
        if (total > 100.001)
            ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'La suma de porcentajes no puede superar 100%.' });
    }),
});
...
function conditionActionPayload(body) {
    if (typeof body === 'object' && body !== null && 'data' in body) {
        const raw = body.data;
        if (typeof raw !== 'string')
            throw new ValidationError('Los datos de la novedad no son válidos.');
        try {
            return conditionActionSchema.parse(JSON.parse(raw));
        }
        catch (error) {
            if (error instanceof SyntaxError)
                throw new ValidationError('Los datos de la novedad no contienen un JSON válido.');
            throw error;
        }
    }
...
function createPayload(body) {
    if (typeof body === 'object' && body !== null && 'data' in body) {
        const raw = body.data;
        if (typeof raw !== 'string')
            throw new ValidationError('Los datos del animal no son válidos.');
        try {
            return createSchema.parse(JSON.parse(raw));
        }
        catch (error) {
            if (error instanceof SyntaxError)
                throw new ValidationError('Los datos del animal no contienen un JSON válido.');
            throw error;
        }
    }
...
function sharedAnimalUrl(token) {
...
animalsRouter.get('/', requirePermission('ANIMAL_CONSULTAR'), asyncHandler(async (req, res) => {
    ...
    if (p.sexo)
        add('a.sexo=?', p.sexo);
    if (p.estado)
        add('a.estado=?', p.estado);
    if (p.id_grupo)
        add('a.id_grupo_actual=?', p.id_grupo);
    if (p.id_ubicacion)
        add('a.id_ubicacion_actual=?', p.id_ubicacion);
    if (p.id_especie)
    ...
    if (p.id_marquilla)
        add('a.id_marquilla=?', p.id_marquilla);
    if (p.nacimiento_desde)
        add('a.fecha_nacimiento>=?', p.nacimiento_desde);
    if (p.nacimiento_hasta)
        add('a.fecha_nacimiento<=?', p.nacimiento_hasta);
    params.push(p.limit, offset(p.page, p.limit));
    const limitIndex = params.length - 1;
    const offsetIndex = params.length;
    const result = await pool.query(`SELECT a.*,a.fecha_nacimiento::text fecha_nacimiento,e.nombre especie,oa.nombre origen,ca.nombre categoria,ca.codigo categoria_codigo,coa.nombre condicion,
...
animalsRouter.get('/opciones/propietarios', requirePermission('ANIMAL_CONSULTAR', 'ANIMAL_CREAR', 'ANIMAL_MODIFICAR'), asyncHandler(async (_req, res) => {
    const rows = (await pool.query(`SELECT id_usuario,TRIM(CONCAT(nombres,' ',apellidos)) nombre,correo
     FROM usuario
...
animalsRouter.get('/novedades', requirePermission('ANIMAL_CONSULTAR'), asyncHandler(async (_req, res) => {
    const rows = (await pool.query(`SELECT * FROM (
     ...
     ORDER BY fecha DESC,animal`)).rows;
    return ok(res, rows);
}));
...
animalsRouter.get('/:id', requirePermission('ANIMAL_CONSULTAR'), asyncHandler(async (req, res) => {
    const result = await pool.query(`SELECT a.*,a.fecha_nacimiento::text fecha_nacimiento,e.nombre especie,oa.nombre origen,ca.nombre categoria,ca.codigo categoria_codigo,coa.nombre condicion,g.nombre grupo,u.nombre ubicacion,m.nombre madre,p.nombre padre,
      fn_clasificacion_animal(a.id_animal,CURRENT_DATE) clasificacion_codigo,
      mq.nombre marquilla,mq.codigo marquilla_codigo,mq.secure_url marquilla_foto,
      COALESCE((SELECT string_agg(TRIM(CONCAT(mu_u.nombres,' ',mu_u.apellidos)),', ' ORDER BY mu.es_principal DESC,mu_u.nombres,mu_u.apellidos)
       FROM marquilla_usuario mu JOIN usuario mu_u ON mu_u.id_usuario=mu.id_usuario AND mu_u.deleted_at IS NULL
     ...
     LEFT JOIN marquilla mq ON mq.id_marquilla=a.id_marquilla AND mq.deleted_at IS NULL
     WHERE a.id_animal=$1 AND a.deleted_at IS NULL`, [routeParam(req.params.id, 'id')]);
    if (!result.rows[0])
        throw new NotFoundError();
    return ok(res, result.rows[0]);
}));
async function assertGenealogyRules(client, input) {
    if (input.id_padre) {
        if (input.id_padre === input.id_animal)
            throw new ValidationError('Un animal no puede registrarse como su propio padre.');
        const father = (await client.query(`SELECT id_especie,sexo,fecha_nacimiento
       FROM animal
       WHERE id_animal=$1 AND deleted_at IS NULL AND estado='ACTIVO'`, [input.id_padre])).rows[0];
        if (!father || father.sexo !== 'MACHO')
            throw new ValidationError('El padre seleccionado no existe o no es un macho activo.');
        if (father.id_especie !== input.id_especie)
            throw new ValidationError('El padre debe pertenecer a la misma especie.');
        if (father.fecha_nacimiento) {
            ...
            if (!age.rows[0]?.valido)
                throw new ValidationError('El padre debe tener al menos un año de edad.');
        }
    }
    if (input.id_madre) {
        if (input.id_madre === input.id_animal)
            throw new ValidationError('Un animal no puede registrarse como su propia madre.');
        const mother = (await client.query(`SELECT id_especie,sexo FROM animal WHERE id_animal=$1 AND deleted_at IS NULL`, [input.id_madre])).rows[0];
        if (!mother || mother.sexo !== 'HEMBRA')
            throw new ValidationError('La madre seleccionada no existe o no es hembra.');
        if (mother.id_especie !== input.id_especie)
            throw new ValidationError('La madre debe pertenecer a la misma especie.');
    }
}
async function assertCategoryLocation(client, categoryId, locationId) {
    const category = await client.query('SELECT 1 FROM categoria_animal WHERE id_categoria_animal=$1 AND deleted_at IS NULL AND activo=TRUE', [categoryId]);
...
async function assertAnimalCondition(client, code, allowInactive = false) {
    ...
    if (!condition.rowCount)
        throw new ValidationError('La condición seleccionada no está disponible.');
}
async function saveRelations(client, id, input) {
    if (input.colores) {
    ...
    if (input.propietarios) {
        await client.query('UPDATE animal_propietario SET fecha_hasta=CURRENT_DATE WHERE id_animal=$1 AND fecha_hasta IS NULL AND deleted_at IS NULL', [id]);
...
animalsRouter.post('/', requirePermission('ANIMAL_CREAR'), createUpload.single('foto_perfil'), asyncHandler(async (req, res) => {
    const input = createPayload(req.body);
    const idAnimal = randomUUID();
    const profilePhoto = req.file;
    let cloud = null;
    if (profilePhoto)
        cloud = await uploadAnimalImage(profilePhoto.buffer, idAnimal);
    try {
        const result = await transaction(async (client) => {
            const { colores, razas, propietarios, peso_inicial_kg, fecha_pesaje_inicial, metodo_pesaje_inicial, observaciones_pesaje_inicial, descripcion_foto_perfil, ...animal } = input;
            ...
            if (cloud) {
                ...
                await client.query(buildInsert('animal_imagen_relacion', {
                    id_imagen: profileImage.id_imagen,
                    id_animal: idAnimal,
                    registrado_por: req.user.id,
                }));
            }
...
animalsRouter.post('/:id/condicion', requirePermission('ANIMAL_MODIFICAR'), conditionUpload.single('imagen'), asyncHandler(async (req, res) => {
    ...
    if (req.file && input.accion !== 'REGISTRAR_HALLAZGO')
        throw new ValidationError('La fotografía solo se admite al registrar la recuperación del animal.');
    const eventId = randomUUID();
    let cloud = null;
    if (req.file)
        cloud = await uploadRecordImage(req.file.buffer, 'novedades', eventId);
    try {
        const result = await transaction(async (client) => {
...
animalsRouter.get('/:id/ubicacion-historica', requirePermission('ANIMAL_CONSULTAR'), asyncHandler(async (req, res) => {
    ...
    const [animalResult, locationResult, groupResult, snapshotResult] = await Promise.all([
        pool.query(`SELECT a.id_animal,a.nombre,a.created_at,a.created_at::date::text fecha_registro,a.id_ubicacion_actual,a.id_grupo_actual
       FROM animal a WHERE a.id_animal=$1 AND a.deleted_at IS NULL`, [id]),
        pool.query(`SELECT h.fecha_desde,h.fecha_hasta,u.id_ubicacion,u.nombre ubicacion,u.tipo tipo_ubicacion,
       ...
       ORDER BY h.fecha_desde DESC LIMIT 1`, [id, fecha]),
        pool.query(`SELECT h.fecha_desde,h.fecha_hasta,g.id_grupo,g.nombre grupo,
         p.id_propiedad,p.nombre propiedad
    ...
    if (!animal)
        throw new NotFoundError('Animal no encontrado.');
    if (fecha < animal.fecha_registro)
        return ok(res, {
            fecha, encontrado: false, propiedad: null, id_propiedad: null, ubicacion: null, id_ubicacion: null, tipo_ubicacion: null,
            grupo: null, id_grupo: null, periodo_ubicacion: null, periodo_grupo: null, estado: null, ultimo_pesaje: null,
            ultimo_tratamiento: null, reproduccion: { ultimo_celo: null, prenez: null, ultimo_parto: null, ultimo_aborto: null, total_partos: 0 },
            lactancia: null, produccion_dia: 0, registros_produccion: 0, total_tratamientos: 0,
        });
...
animalsRouter.get('/:id/compartir', requirePermission('ANIMAL_MODIFICAR'), asyncHandler(async (req, res) => {
    const id = routeParam(req.params.id, 'id');
    const row = (await pool.query(`SELECT token,created_at FROM animal_compartido
     WHERE id_animal=$1 AND activo=TRUE AND revocado_at IS NULL
     ORDER BY created_at DESC LIMIT 1`, [id])).rows[0];
...
animalsRouter.post('/:id/compartir', requirePermission('ANIMAL_MODIFICAR'), asyncHandler(async (req, res) => {
    const id = routeParam(req.params.id, 'id');
    const result = await transaction(async (client) => {
        const animal = (await client.query('SELECT id_animal FROM animal WHERE id_animal=$1 AND deleted_at IS NULL FOR SHARE', [id])).rows[0];
        if (!animal)
            throw new NotFoundError('Animal no encontrado.');
        const current = (await client.query(`SELECT token,created_at FROM animal_compartido
       WHERE id_animal=$1 AND activo=TRUE AND revocado_at IS NULL
       ORDER BY created_at DESC LIMIT 1 FOR UPDATE`, [id])).rows[0];
        if (current)
            return current;
        const token = randomUUID();
        return (await client.query(`INSERT INTO animal_compartido(id_animal,token,creado_por)
       VALUES($1,$2,$3) RETURNING token,created_at`, [id, token, req.user.id])).rows[0];
    }, req.user.id);
...
animalsRouter.delete('/:id/compartir', requirePermission('ANIMAL_MODIFICAR'), asyncHandler(async (req, res) => {
    const result = await pool.query(`UPDATE animal_compartido
     SET activo=FALSE,revocado_at=NOW(),updated_at=NOW()
     WHERE id_animal=$1 AND activo=TRUE AND revocado_at IS NULL`, [routeParam(req.params.id, 'id')]);
    if (!result.rowCount)
        throw new NotFoundError('El animal no tiene un enlace público activo.');
    return noContent(res);
}));
...
animalsRouter.delete('/:id', requirePermission('ANIMAL_ELIMINAR'), asyncHandler(async (req, res) => {
    const result = await pool.query("UPDATE animal SET deleted_at=NOW(),estado='INACTIVO' WHERE id_animal=$1 AND deleted_at IS NULL", [routeParam(req.params.id, 'id')]);
    if (!result.rowCount)
        throw new NotFoundError();
    return noContent(res);
}));
