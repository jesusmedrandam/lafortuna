...
animalsRouter.get('/', requirePermission('ANIMAL_CONSULTAR'), asyncHandler(async (req, res) => {
    ...
    if (p.id_propiedad)
        add('COALESCE(g.id_propiedad,u.id_propiedad)=?', p.id_propiedad);
    if (p.id_especie)
    ...
    const result = await pool.query(`SELECT a.*,e.nombre especie,ca.nombre categoria,ca.codigo categoria_codigo,coa.nombre condicion,g.nombre grupo,u.nombre ubicacion,
