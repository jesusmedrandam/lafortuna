...
propertiesRouter.post('/', requirePermission('UBICACION_ADMINISTRAR'), asyncHandler(async (req, res) => {
    ...
    const row = await transaction(async (client) => {
        const hasPrincipal = Boolean((await client.query('SELECT 1 FROM propiedad_ganadera WHERE es_principal=TRUE AND deleted_at IS NULL LIMIT 1')).rowCount);
        const principal = input.es_principal ?? !hasPrincipal;
        if (principal) {
            await client.query('UPDATE propiedad_ganadera SET es_principal=FALSE,updated_at=NOW() WHERE es_principal=TRUE AND deleted_at IS NULL');
        }
...
propertiesRouter.patch('/:id', requirePermission('UBICACION_ADMINISTRAR'), asyncHandler(async (req, res) => {
    ...
    const row = await transaction(async (client) => {
        const current = (await client.query('SELECT * FROM propiedad_ganadera WHERE id_propiedad=$1 AND deleted_at IS NULL FOR UPDATE', [id])).rows[0];
        if (!current)
        ...
        if (input.es_principal === true) {
            await client.query(`UPDATE propiedad_ganadera SET es_principal=FALSE,updated_at=NOW()
         WHERE id_propiedad<>$1 AND es_principal=TRUE AND deleted_at IS NULL`, [id]);
            input.activa = true;
        }
...
propertiesRouter.delete('/:id', requirePermission('UBICACION_ADMINISTRAR'), asyncHandler(async (req, res) => {
    ...
    await transaction(async (client) => {
        const current = (await client.query('SELECT es_principal FROM propiedad_ganadera WHERE id_propiedad=$1 AND deleted_at IS NULL FOR UPDATE', [id])).rows[0];
        if (!current)
            throw new NotFoundError('Propiedad no encontrada.');
        if (current.es_principal)
            throw new ConflictError('La propiedad principal no puede eliminarse.');
        const related = await client.query(`SELECT
        EXISTS(SELECT 1 FROM grupo WHERE id_propiedad=$1 AND deleted_at IS NULL) grupos,
        EXISTS(SELECT 1 FROM ubicacion WHERE id_propiedad=$1 AND tipo IN('POTRERO','CORRAL') AND deleted_at IS NULL) ubicaciones`, [id]);
        ...
        await client.query('UPDATE propiedad_ganadera SET activa=FALSE,deleted_at=NOW(),updated_at=NOW() WHERE id_propiedad=$1', [id]);
    }, req.user.id);
