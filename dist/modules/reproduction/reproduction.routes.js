...
async function assistedServiceData(client, input) {
    ...
    const activePregnancy = await client.query(`SELECT 1 FROM prenez
     WHERE id_vaca=$1 AND estado='CONFIRMADA' AND deleted_at IS NULL
     LIMIT 1 FOR SHARE`, [input.id_vaca]);
    if (activePregnancy.rowCount)
        throw new ValidationError('No se puede realizar una inseminación o transferencia de embriones porque la receptora ya tiene una preñez confirmada.');
