...
`)).rows)));
cleaningsRouter.post('/', requirePermission('LIMPIEZA_ADMINISTRAR'), asyncHandler(async (req, res) => { const x = schema.parse(req.body); const result = await transaction(async (c) => { const { productos, operadores, ...head } = x; const l = (await c.query(buildInsert('limpieza_potrero', { ...head, registrado_por: req.user.id }))).rows[0]; for (const p of productos)
    await c.query(buildInsert('limpieza_potrero_producto', { ...p, id_limpieza: l.id_limpieza })); for (const o of operadores)
    await c.query(buildInsert('limpieza_potrero_operador', { ...o, id_limpieza: l.id_limpieza })); return l; }, req.user.id); return created(res, result); }));
cleaningsRouter.patch('/:id', requirePermission('LIMPIEZA_ADMINISTRAR'), asyncHandler(async (req, res) => { const id = routeParam(req.params.id, 'id'); const x = schema.parse(req.body); const result = await transaction(async (c) => { const found = (await c.query('SELECT id_limpieza FROM limpieza_potrero WHERE id_limpieza=$1 AND deleted_at IS NULL FOR UPDATE', [id])).rows[0]; if (!found)
    throw new NotFoundError('Limpieza no encontrada.'); const { productos, operadores, ...head } = x; const row = (await c.query(`UPDATE limpieza_potrero SET id_potrero=$2,id_tipo_limpieza=$3,fecha_inicio=$4,fecha_finalizacion=$5,unidad_aplicacion=$6,cantidad_tanques=$7,capacidad_tanque_litros=$8,area_intervenida=$9,id_unidad_area=$10,estado=$11,observaciones=$12,updated_at=NOW() WHERE id_limpieza=$1 RETURNING *`, [id, head.id_potrero, head.id_tipo_limpieza, head.fecha_inicio, head.fecha_finalizacion ?? null, head.unidad_aplicacion, head.cantidad_tanques ?? null, head.capacidad_tanque_litros ?? null, head.area_intervenida ?? null, head.id_unidad_area ?? null, head.estado, head.observaciones ?? null])).rows[0]; await c.query('DELETE FROM limpieza_potrero_producto WHERE id_limpieza=$1', [id]); await c.query('DELETE FROM limpieza_potrero_operador WHERE id_limpieza=$1', [id]); for (const p of productos)
    await c.query(buildInsert('limpieza_potrero_producto', { ...p, id_limpieza: id })); for (const o of operadores)
    await c.query(buildInsert('limpieza_potrero_operador', { ...o, id_limpieza: id })); return row; }, req.user.id); return ok(res, result); }));
