...
operatorsRouter.patch('/:id', requirePermission('LIMPIEZA_ADMINISTRAR'), asyncHandler(async (req, res) => { const row = (await pool.query(buildUpdate('operador', 'id_operador', routeParam(req.params.id, 'id'), schema.partial().parse(req.body)))).rows[0]; if (!row)
    throw new NotFoundError(); return ok(res, row); }));
operatorsRouter.delete('/:id', requirePermission('LIMPIEZA_ADMINISTRAR'), asyncHandler(async (req, res) => { const r = await pool.query('UPDATE operador SET deleted_at=NOW(),activo=FALSE WHERE id_operador=$1 AND deleted_at IS NULL', [routeParam(req.params.id, 'id')]); if (!r.rowCount)
