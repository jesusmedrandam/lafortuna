...
export async function notifyCleaning(database: Queryable, row: Row, actor: string) {
  ...
  const cleaningTypes = (await database.query(
    `SELECT tl.nombre
     ...
     ORDER BY tl.nombre`, [row.id_limpieza],
  )).rows as Row[];
  const products = (await database.query(
    `SELECT pa.nombre_comercial producto,lp.cantidad_total,COALESCE(um.simbolo,um.nombre) unidad
     ...
     ORDER BY pa.nombre_comercial`, [row.id_limpieza],
  )).rows as Row[];
