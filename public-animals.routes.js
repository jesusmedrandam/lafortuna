...
publicAnimalsRouter.get('/:token', asyncHandler(async (req, res) => {
    ...
    const result = await pool.query(`SELECT a.id_animal,a.nombre,a.codigo_arete,a.descripcion,a.sexo,
       ...
       WHERE gallery.deleted_at IS NULL
         AND gallery.es_perfil=FALSE
         AND COALESCE(gallery.tipo_archivo,'IMAGEN')='IMAGEN'
     ...
     LIMIT 1`, [token]);
    if (!result.rows[0])
        throw new NotFoundError('Esta ficha pública no existe o dejó de estar disponible.');
    res.setHeader('Cache-Control', 'public, max-age=60, stale-while-revalidate=300');
    return ok(res, result.rows[0]);
}));
//# sourceMappingURL=public-animals.routes.js.map