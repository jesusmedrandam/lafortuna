...
export async function notifyCleaning(database, row, actor) {
    ...
    const cleaningTypes = (await database.query(`SELECT tl.nombre
     FROM limpieza_potrero_actividad la
     JOIN tipo_limpieza_potrero tl ON tl.id_tipo_limpieza=la.id_tipo_limpieza
     WHERE la.id_limpieza=$1 AND la.deleted_at IS NULL
     ORDER BY tl.nombre`, [row.id_limpieza])).rows;
    const products = (await database.query(`SELECT pa.nombre_comercial producto,lp.cantidad_total,COALESCE(um.simbolo,um.nombre) unidad
     ...
     WHERE lp.id_limpieza=$1 AND lp.deleted_at IS NULL
     ORDER BY pa.nombre_comercial`, [row.id_limpieza])).rows;
    const state = text(row.estado, 'COMPLETADO');
    const typeSummary = cleaningTypes.map(item => text(item.nombre)).filter(Boolean).join(', ') || 'Limpieza';
    const applications = number(row.cantidad_tanques);
    const applicationUnit = text(row.unidad_aplicacion) === 'BOMBADAS'
        ? countLabel(applications, 'bombada', 'bombadas')
        : countLabel(applications, 'tanque', 'tanques');
    const productSummary = products.map(product => `${text(product.producto)} · ${decimal(product.cantidad_total, 4)} ${text(product.unidad)} en total`);
    const messageParts = [typeSummary, applications > 0 ? applicationUnit : '', ...productSummary].filter(Boolean);
    return emitBusinessNotification(database, actor, text(row.id_limpieza), {
        ...
        prioridad: state === 'COMPLETADO' ? 'INFO' : 'IMPORTANTE', titulo: `${text(pasture?.nombre, 'Potrero')} recibió limpieza`,
        ...
        datos: { id_potrero: row.id_potrero, estado: state, tipos: cleaningTypes.map(item => text(item.nombre)), productos: products },
    });
}
