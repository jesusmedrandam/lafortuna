...
export async function reproductionRulesForAnimal(client, animalId) {
    const row = (await client.query(`SELECT property.id_propiedad,
     ...
     WHERE a.id_animal=$1 AND a.deleted_at IS NULL`, [animalId])).rows[0];
    return row ?? defaults;
}
async function latestEventDate(client, table, animalColumn, animalId, eventDate) {
    const dateExpression = table === 'parto' ? 'fecha_parto' : table === 'aborto' ? 'COALESCE(fecha,created_at::date)' : 'fecha';
    return (await client.query(`SELECT fecha::text FROM (SELECT ${dateExpression} AS fecha
       FROM ${table} WHERE ${animalColumn}=$1 AND deleted_at IS NULL
         AND ${dateExpression}<=$2::date
       ORDER BY ${dateExpression} DESC LIMIT 1) event`, [animalId, eventDate])).rows[0]?.fecha;
}
async function assertDaysSince(client, label, eventDate, lastDate, minimum) {
    if (!lastDate || minimum <= 0)
        return;
    const elapsed = Number((await client.query('SELECT ($1::date-$2::date)::int days', [eventDate, lastDate])).rows[0]?.days ?? 0);
    if (elapsed < minimum)
        throw new ValidationError(`${label}: deben transcurrir ${minimum} días; han pasado ${elapsed}.`);
}
export async function assertFemaleReproductionRules(client, animalId, eventDate, kind, falseHeat = false, excludeHeatId, excludePregnancyId) {
    ...
    if (kind === 'CELO') {
        if (activePregnancy.rowCount && !falseHeat)
            throw new ValidationError('La vaca tiene una preñez confirmada. Solo se puede registrar un celo si se marca como falso.');
        if (falseHeat && !rules.permitir_celo_falso_en_prenez)
            throw new ValidationError('La configuración de la propiedad no permite registrar celos falsos.');
        if (!rules.permitir_segundo_celo && Number(heatCount.rows[0]?.total ?? 0) > 0)
            throw new ValidationError('La configuración de la propiedad no permite registrar un segundo celo en este ciclo.');
...
export async function assertMinimumAge(client, birthDate, eventDate, months, role) {
    if (!birthDate || months <= 0)
        return;
    ...
    if (!allowed)
        throw new ValidationError(`${role} debe tener al menos ${months} meses de edad en la fecha seleccionada.`);
}
//# sourceMappingURL=reproduction-policy.js.map