...
export function PersonalFinancePage() {
  ...
  return <div className="personal-finance-page module-no-header">
    ...
    {tab === 'CUENTAS' ? <section className="finance-section"><div className="section-heading-inline"><div><h2>Saldos y cuentas</h2><p className="muted">No se solicitan números de cuenta ni información bancaria personal.</p></div><Button onClick={() => setAccountForm(emptyAccount())}><Plus size={17} />Cuenta</Button></div>
      {accounts.data?.length ? <div className="finance-account-grid detailed">{accounts.data.map((account) => { const Icon = accountIcon(account.tipo), balance = balances.get(account.id_cuenta) ?? 0, isCredit = account.tipo === 'TARJETA_CREDITO'; return <Card key={account.id_cuenta} className={`finance-account-detail ${account.activa ? '' : 'inactive'}`}><div className="finance-account-detail-head"><span className="finance-account-icon" style={{ background: account.color ?? accountColors[0] }}><Icon size={22} /></span><span><small>{isCredit ? 'Tarjeta de crédito' : humanizeCode(account.tipo)}</small><strong>{account.nombre}</strong></span><Badge tone={account.activa ? 'success' : 'neutral'}>{account.activa ? 'Activa' : 'Inactiva'}</Badge></div><strong className="finance-account-balance">{showMoney(isCredit ? Math.max(0, -balance) : balance)}</strong><small>{isCredit ? `Deuda actual · disponible ${showMoney(Math.max(0, amount(account.limite_credito) + balance))}` : account.descripcion || 'Sin descripción'}</small>{isCredit && (account.dia_corte || account.dia_pago) ? <small>Corte: día {account.dia_corte || '—'} · pago: día {account.dia_pago || '—'}</small> : null}<div className="finance-inline-actions">{!isCredit ? <Button variant="secondary" onClick={() => { setAdjusting(account); setTargetBalance(String(balance.toFixed(2))); }}>Ajustar saldo</Button> : null}<IconButton label="Editar cuenta" onClick={() => setAccountForm({ id: account.id_cuenta, nombre: account.nombre, tipo: account.tipo, saldo_inicial: String(Math.abs(amount(account.saldo_inicial))), descripcion: account.descripcion ?? '', color: account.color ?? accountColors[0], activa: account.activa, limite_credito: account.limite_credito == null ? '' : String(account.limite_credito), dia_corte: account.dia_corte == null ? '' : String(account.dia_corte), dia_pago: account.dia_pago == null ? '' : String(account.dia_pago) })}><Edit3 size={17} /></IconButton><IconButton label="Eliminar cuenta" onClick={() => setDeleting({ kind: 'cuenta', id: account.id_cuenta, label: account.nombre })}><Trash2 size={17} /></IconButton></div></Card>; })}</div> : <EmptyState icon={WalletCards} title="Sin cuentas" description="Registra dónde mantienes tu dinero." />}
    </section> : null}
