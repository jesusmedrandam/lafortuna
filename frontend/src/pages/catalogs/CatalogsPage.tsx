...
const catalogDefinitions = [
  ...
  ['condiciones-animales', 'Condiciones del animal'], ['especies', 'Especies'], ['origenes', 'Orígenes'], ['colores', 'Colores'], ['razas', 'Razas'],
  ...
  return <div>
    ...
    <div className="catalog-layout">
      ...
      <section className="catalog-content">
        <div className="section-heading-inline catalog-heading"><div><h2>{title}</h2><p className="muted">{query.data?.length ?? 0} elementos registrados</p></div><div className="inline-actions">{hasPermission('UBICACION_CONSULTAR') ? <Button variant="ghost" onClick={() => navigate('/propiedades')}>Propiedades</Button> : null}{hasPermission('CATALOGO_ADMINISTRAR') ? <Button onClick={() => { setEditingId(null); setForm(emptyForm()); setOpen(true); }}><Plus size={17} />Agregar</Button> : null}</div></div>
