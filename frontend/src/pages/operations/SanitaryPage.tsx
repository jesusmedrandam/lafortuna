...
export function SanitaryPage() {
  ...
  const campaignList = useListControls({ items: campaigns.data ?? [], storageKey: 'sanitary-campaigns', searchText: (item) => `${item.tipo_tratamiento} ${item.medicamento} ${item.via} ${item.responsable ?? ''} ${item.detalles.map((detail) => detail.animal).join(' ')}`, dateValue: (item) => item.fecha_aplicacion, nameValue: (item) => item.tipo_tratamiento });
  const treatmentList = useListControls({ items: treatments.data ?? [], storageKey: 'sanitary-treatments', searchText: (item) => `${String(item.animal ?? '')} ${String(item.codigo_arete ?? '')} ${typeName(item.id_tipo_tratamiento)} ${medicineName(item.id_medicamento)}`, dateValue: (item) => String(item.fecha_aplicacion ?? ''), nameValue: (item) => String(item.animal ?? '') });
  const controls = tab === 'campaigns' ? campaignList : treatmentList;
  const editCampaign = (item: SanitaryCampaign) => {
    const date = new Date(item.fecha_aplicacion); date.setMinutes(date.getMinutes() - date.getTimezoneOffset());
    setCampaign({ id_tipo_tratamiento: item.id_tipo_tratamiento, id_medicamento: item.id_medicamento, id_via_administracion: item.id_via_administracion, dosis_general: String(item.dosis_general), id_unidad_dosis: item.id_unidad_dosis, fecha_aplicacion: date.toISOString().slice(0, 16), responsable: item.responsable ?? '', observaciones: item.observaciones ?? '', selection: { mode: item.modo_seleccion, groupId: item.id_grupo_filtro ?? '', animals: item.detalles.map((detail) => ({ id_animal: detail.id_animal, nombre: detail.nombre ?? detail.animal, codigo_arete: detail.codigo_arete ?? null, sexo: detail.sexo ?? 'HEMBRA', id_grupo_actual: detail.id_grupo_actual ?? null, grupo: detail.grupo ?? null, id_ubicacion_actual: detail.id_ubicacion_actual ?? null, ubicacion: detail.ubicacion ?? null, seleccionado: detail.seleccionado, dosis_aplicada: detail.dosis_aplicada == null ? null : Number(detail.dosis_aplicada), id_unidad_dosis: detail.id_unidad_dosis ?? item.id_unidad_dosis, observaciones: detail.observaciones ?? null })) } });
