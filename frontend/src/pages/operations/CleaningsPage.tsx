...
export function CleaningsPage() {
  ...
  const editCleaning = (item: PastureCleaning) => {
    const toLocal = (value: string | null) => { if (!value) return ''; const date = new Date(value); date.setMinutes(date.getMinutes() - date.getTimezoneOffset()); return date.toISOString().slice(0, 16); };
    setForm({ id_potrero: item.id_potrero, id_tipo_limpieza: item.id_tipo_limpieza, fecha_inicio: toLocal(item.fecha_inicio), fecha_finalizacion: toLocal(item.fecha_finalizacion), unidad_aplicacion: item.unidad_aplicacion ?? 'TANQUES', cantidad_tanques: item.cantidad_tanques == null ? '' : String(item.cantidad_tanques), capacidad_tanque_litros: item.capacidad_tanque_litros == null ? '' : String(item.capacidad_tanque_litros), area_intervenida: item.area_intervenida == null ? '' : String(item.area_intervenida), id_unidad_area: item.id_unidad_area ?? '', estado: item.estado, observaciones: item.observaciones ?? '', productos: item.productos.map((product) => ({ id_producto: product.id_producto, cantidad_total: String(product.cantidad_total), id_unidad: product.id_unidad, cantidad_por_tanque: product.cantidad_por_tanque == null ? '' : String(product.cantidad_por_tanque), observaciones: product.observaciones ?? '' })), operadores: item.operadores.map((operator) => ({ id_operador: operator.id_operador, funcion: operator.funcion ?? '', horas_trabajadas: operator.horas_trabajadas == null ? '' : String(operator.horas_trabajadas), observaciones: operator.observaciones ?? '' })) });
