...
export function CleaningsPage() {
  ...
  return <div>
    ...
    {cleanings.isLoading ? <LoadingState /> : cleanings.isError ? <ErrorState message={(cleanings.error as Error).message} onRetry={() => void cleanings.refetch()} /> : cleanings.data?.length ? <div className="record-grid operation-grid">{cleanings.data.map((item) => { const applicationLabel = item.unidad_aplicacion === 'BOMBADAS' ? 'Bombadas' : item.unidad_aplicacion === 'TANQUES' ? 'Tanques' : 'Tanques o bombadas'; const perApplicationLabel = item.unidad_aplicacion === 'BOMBADAS' ? 'bombada' : item.unidad_aplicacion === 'TANQUES' ? 'tanque' : 'aplicación'; return <Card className="operation-card" key={item.id_limpieza}>
      ...
      <div className="detail-grid compact"><div><small>{applicationLabel}</small><strong>{item.cantidad_tanques == null ? '—' : formatNumber(item.cantidad_tanques)}</strong></div><div><small>Capacidad por {perApplicationLabel}</small><strong>{item.capacidad_tanque_litros == null ? '—' : `${formatNumber(item.capacidad_tanque_litros)} L`}</strong></div><div><small>Área intervenida</small><strong>{item.area_intervenida == null ? '—' : formatNumber(item.area_intervenida)}</strong></div><div><small>Finalización</small><strong>{formatDateTime(item.fecha_finalizacion)}</strong></div></div>
      {item.productos?.length ? <details className="operation-details"><summary>Productos aplicados ({item.productos.length})</summary><div>{item.productos.map((product, index) => <span key={`${item.id_limpieza}-p-${index}`}>{product.producto}: {formatNumber(product.cantidad_total, 4)} {product.unidad}{product.cantidad_por_tanque ? ` · ${formatNumber(product.cantidad_por_tanque, 4)} por ${perApplicationLabel}` : ''}</span>)}</div></details> : null}
