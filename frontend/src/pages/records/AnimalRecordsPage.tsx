...
export function AnimalRecordsPage({ mode }: { mode: Mode }) {
  ...
  return <div><PageHeader title={title} description={description} action={hasPermission(permission) ? <Button onClick={() => { setForm(emptyForm()); setOpen(true); }}><Plus size={18} />Nuevo registro</Button> : undefined} />
    ...
    {records.isLoading ? <LoadingState /> : records.isError ? <ErrorState message={(records.error as Error).message} onRetry={() => void records.refetch()} /> : visibleRecords.length ? <div className="table-card"><div className="table-responsive"><table className="data-table"><thead><tr><th>Animal</th><th>Fecha</th>{mode === 'pesajes' ? <><th>Peso</th><th>Método</th><th>Observaciones</th></> : <><th>Causa</th><th>Descripción</th></>}{hasPermission(permission) ? <th>Acciones</th> : null}</tr></thead><tbody>{visibleRecords.map((record) => <tr key={String(record[idField])}><td><strong>{String(record.animal ?? '—')}</strong><small>{record.codigo_arete ? `Arete ${record.codigo_arete}` : ''}</small></td><td>{formatDate(String(mode === 'pesajes' ? record.fecha_pesaje : record.fecha))}</td>{mode === 'pesajes' ? <><td><strong>{formatNumber(record.peso_kg as number | string, 3)} kg</strong></td><td>{String(record.metodo ?? '—')}</td><td>{String(record.observaciones ?? '—')}</td></> : <><td>{String(record.causa ?? '—')}</td><td>{String(record.descripcion ?? '—')}</td></>}{hasPermission(permission) ? <td><div className="inline-actions"><Button variant="ghost" onClick={() => edit(record)}><Edit3 size={16} /></Button><Button variant="ghost" onClick={() => setDeleteId(String(record[idField]))}><Trash2 size={16} /></Button></div></td> : null}</tr>)}</tbody></table></div></div> : <EmptyState icon={Icon} title={`Sin ${mode === 'pesajes' ? 'pesajes' : 'muertes'} registrados`} description={description} />}
