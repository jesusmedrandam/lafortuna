...
import { Edit3, HeartOff, ImagePlus, MapPin, Plus, Search, Trash2, Weight } from 'lucide-react';
...
import type { Animal, AnimalStatusNews, GenericRecord } from '../../types/api';
...
export function AnimalRecordsPage({ mode }: { mode: Mode }) {
  ...
  const visibleNews=(news.data??[]).filter((item)=>isInOwnershipScope(item.categoria_codigo??'',ownershipScope)&&(newsFilter==='TODOS'||item.tipo===newsFilter));
  ...
  const save = useMutation({
    ...
    onSuccess: () => { toast.show(form.id ? 'Registro actualizado.' : 'Registro creado.'); setOpen(false); setPhoto(null); setForm(emptyForm()); void client.invalidateQueries({ queryKey: ['records', mode] }); void client.invalidateQueries({ queryKey: ['animals'] }); void client.invalidateQueries({queryKey:['animal-status-news']}); },
  ...
  const title = mode === 'pesajes' ? 'Pesajes' : 'Novedades y bajas';
  ...
  return <div><PageHeader title={title} description={description} action={hasPermission(permission) ? <Button onClick={() => { setPhoto(null); setForm(emptyForm()); setOpen(true); }}><Plus size={18} />{mode==='pesajes'?'Nuevo pesaje':'Registrar muerte'}</Button> : undefined} />
    {mode === 'muertes' ? <OwnershipScopeFilter value={ownershipScope} onChange={(scope) => { setOwnershipScope(scope); setOpen(false); setForm(emptyForm()); }} /> : null}
