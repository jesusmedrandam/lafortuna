...
const filterKeys=['categoria','tipo',...contextualKeys,'fecha_desde','fecha_hasta'] as const;
...
export function MultimediaPage(){
  ...
  const beginEdit=(item:MultimediaItem)=>{setEditing(item);setEditIds(item.animales?.map((animal)=>animal.id_animal)??[]);setEditDate(item.fecha_toma||currentDateInput());setEditTags(item.etiquetas?.map((tag)=>tag.id_etiqueta)??[]);};
  ...
  return <div>
    ...
    {viewerIndex!==null?<ImageLightbox items={viewerItems} initialIndex={viewerIndex} onClose={()=>setViewerIndex(null)} actions={(media)=>{const item=items.find((candidate)=>candidate.id_multimedia===media.key);return item?.editable&&hasPermission('IMAGEN_ADMINISTRAR')?<div className="lightbox-actions"><IconButton label="Editar fotografía" onClick={()=>beginEdit(item)}><Pencil size={18}/></IconButton><IconButton className="detail-action-danger" label="Eliminar fotografía" onClick={()=>setDeleteItem(item)}><Trash2 size={18}/></IconButton></div>:null;}}/>:null}
