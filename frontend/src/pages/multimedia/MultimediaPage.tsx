...
export function MultimediaPage(){
  ...
  return <div>
    ...
    {viewerIndex!==null?<ImageLightbox
      ...
      minimalControls
      actions={(media)=>{const item=items.find((candidate)=>candidate.id_multimedia===media.key);return item&&!item.es_perfil&&item.editable&&hasPermission('IMAGEN_ADMINISTRAR')?<div className="lightbox-actions"><IconButton label="Editar relaciones y etiquetas" onClick={()=>{setViewerIndex(null);beginEdit(item);}}><Pencil size={18}/></IconButton></div>:null;}}
    />:null}
