...
import { ChevronRight, Edit3, Plus, SlidersHorizontal, Trash2, Users } from 'lucide-react';
...
export function GroupsPage() {
  ...
  const visibleGroups=(query.data?.data??[]).filter((group)=>statusFilter==='ALL'||(statusFilter==='ACTIVE'?group.activo:!group.activo));
  const cycleStatus=()=>setStatusFilter((current)=>current==='ALL'?'ACTIVE':current==='ACTIVE'?'INACTIVE':'ALL');

  return <div className="module-no-header">
    <CompactToolbar search={search} onSearch={setSearch} placeholder="Buscar grupo…" count={visibleGroups.length} actions={<IconButton className={statusFilter==='ALL'?'':'active'} label={statusFilter==='ALL'?'Todos los grupos':statusFilter==='ACTIVE'?'Solo activos':'Solo inactivos'} onClick={cycleStatus}><SlidersHorizontal size={19}/></IconButton>}/>
    {query.isLoading ? <LoadingState /> : query.isError ? <ErrorState message={(query.error as Error).message} onRetry={() => void query.refetch()} /> : visibleGroups.length === 0 ? <EmptyState icon={Users} title="Sin grupos" description="Crea un grupo o modifica los filtros." /> : <div className="record-grid">
      {visibleGroups.map((group) => <Card key={group.id_grupo} className="record-card" onClick={()=>setSelectedId(group.id_grupo)}>
