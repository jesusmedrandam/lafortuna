...
function ProductSaleForm({ sale, onClose, onSaved }: { sale?: ProductSale; onClose: () => void; onSaved: () => void }) {
  ...
  const [form, setForm] = useState({ fecha_venta: sale ? dateInputValue(sale.fecha_venta) : currentDateInput(), periodicidad: sale?.periodicidad ?? 'DIARIA' as 'DIARIA' | 'SEMANAL', id_comprador: sale?.id_comprador ?? '', observaciones: sale?.observaciones ?? '', productos: sale?.productos.map((item) => ({ id_producto_venta: item.id_producto_venta, cantidad: String(item.cantidad), cantidad_complementaria:item.cantidad_complementaria==null?'':String(item.cantidad_complementaria), precio_unitario: String(item.precio_unitario), observaciones: item.observaciones ?? '' })) ?? [emptyProductLine()] });
  const selectedBuyer = buyers.data?.find((item) => itemId(item) === form.id_comprador);
  const selectBuyer = (id: string) => setForm((current) => ({ ...current, id_comprador: id }));
