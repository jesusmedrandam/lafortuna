...
import type { AnimalSale, ProductSale } from '../../types/api';
...
function AnimalSaleForm({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const mutation = useMutation({ mutationFn: () => { const selected = selection.animals.filter((animal) => animal.seleccionado); if (!selected.length) throw new ApiError(400, 'NO_ANIMALS', 'Selecciona al menos un animal.'); return apiRequest('/ventas', { method: 'POST', body: { fecha_venta: new Date(form.fecha_venta).toISOString(), comprador_nombre: form.comprador_nombre.trim(), comprador_contacto: nullIfEmpty(form.comprador_contacto), destino: nullIfEmpty(form.destino), precio_total: numberOrNull(form.precio_total), moneda: 'USD', observaciones: nullIfEmpty(form.observaciones), animales: selected.map((animal) => ({ id_animal: animal.id_animal, precio_individual: null, observaciones: null })) } }); }, onSuccess: async () => { toast.show('Venta registrada correctamente.'); await client.invalidateQueries({ queryKey: ['sales'] }); await client.invalidateQueries({ queryKey: ['animals'] }); await client.invalidateQueries({ queryKey: ['dashboard'] }); onSaved(); }, onError: (error) => toast.show((error as ApiError).message, 'error') });
  return <Modal title="Registrar venta de animales" onClose={onClose} wide footer={<><Button variant="ghost" onClick={onClose}>Cancelar</Button><Button type="submit" form="animal-sale-form" loading={mutation.isPending}><ShoppingCart size={18} />Guardar venta</Button></>}><form id="animal-sale-form" className="form-stack" onSubmit={(event: FormEvent) => { event.preventDefault(); mutation.mutate(); }}><div className="form-grid"><Field label="Fecha" required><Input type="datetime-local" required value={form.fecha_venta} onChange={(event) => setForm({ ...form, fecha_venta: event.target.value })} /></Field><Field label="Comprador" required><Input required value={form.comprador_nombre} onChange={(event) => setForm({ ...form, comprador_nombre: event.target.value })} /></Field><Field label="Contacto"><Input value={form.comprador_contacto} onChange={(event) => setForm({ ...form, comprador_contacto: event.target.value })} /></Field><Field label="Destino"><Input value={form.destino} onChange={(event) => setForm({ ...form, destino: event.target.value })} /></Field><Field label="Precio total (USD)"><Input type="number" min="0" step="0.01" value={form.precio_total} onChange={(event) => setForm({ ...form, precio_total: event.target.value })} /></Field></div><Field label="Observaciones"><Textarea rows={3} value={form.observaciones} onChange={(event) => setForm({ ...form, observaciones: event.target.value })} /></Field><div className="form-section"><h3>Animales vendidos</h3><AnimalSelectionBuilder value={selection} onChange={setSelection} /></div></form></Modal>;
}
...
function ProductSaleForm({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  const toast = useToast(); const client = useQueryClient(); const products = useCatalog('productos-venta');
  const [form, setForm] = useState({ fecha_venta: localDateTime(), periodicidad: 'DIARIA' as 'DIARIA' | 'SEMANAL', comprador_nombre: '', comprador_contacto: '', destino: '', observaciones: '', productos: [emptyProductLine()] });
