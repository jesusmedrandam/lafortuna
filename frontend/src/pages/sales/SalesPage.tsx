...
function AnimalSaleForm({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const [form, setForm] = useState({ fecha_venta: currentDateInput(), id_comprador: '', destino: '', precio_total: '', observaciones: '' });
  const selectedBuyer = buyers.data?.find((item) => itemId(item) === form.id_comprador);
...
function ProductSaleForm({ sale, onClose, onSaved }: { sale?: ProductSale; onClose: () => void; onSaved: () => void }) {
  ...
  const [form, setForm] = useState({ fecha_venta: sale ? dateInputValue(sale.fecha_venta) : currentDateInput(), periodicidad: sale?.periodicidad ?? 'DIARIA' as 'DIARIA' | 'SEMANAL', id_comprador: sale?.id_comprador ?? '', destino: sale?.destino ?? '', observaciones: sale?.observaciones ?? '', productos: sale?.productos.map((item) => ({ id_producto_venta: item.id_producto_venta, cantidad: String(item.cantidad), precio_unitario: String(item.precio_unitario), observaciones: item.observaciones ?? '' })) ?? [emptyProductLine()] });
  const selectedBuyer = buyers.data?.find((item) => itemId(item) === form.id_comprador);
  const selectBuyer = (id: string) => { const buyer = buyers.data?.find((item) => itemId(item) === id); setForm((current) => ({ ...current, id_comprador: id, destino: String(buyer?.destino ?? '') })); };
