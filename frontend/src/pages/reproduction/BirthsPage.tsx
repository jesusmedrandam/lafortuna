...
export function BirthsPage() {
  ...
  const saveBirthEdit = useMutation({
    mutationFn: () => {
      ...
      return apiRequest(`/partos/${birthEditForm.id_parto}`, { method: 'PATCH', body: { fecha_parto: new Date(birthEditForm.fecha_parto).toISOString(), tipo_parto: birthEditForm.tipo_parto, observaciones: nullIfEmpty(birthEditForm.observaciones) } });
    },
  ...
  const editBirth = (birth: Birth) => { const date = new Date(birth.fecha_parto); date.setMinutes(date.getMinutes() - date.getTimezoneOffset()); setBirthEditForm({ id_parto: birth.id_parto, fecha_parto: date.toISOString().slice(0, 16), tipo_parto: birth.tipo_parto, observaciones: birth.observaciones ?? '' }); setSelectedBirth(null); };
  ...
  return <div>
    ...
    {tab === 'abortions' ? <AbortionsList query={abortions} items={abortionList.visible} canEdit={hasPermission('ABORTO_ADMINISTRAR')} onEdit={(record) => { const date = record.fecha ? new Date(String(record.fecha)) : null; if (date) date.setMinutes(date.getMinutes() - date.getTimezoneOffset()); setAbortionForm({ id_aborto: String(record.id_aborto), id_vaca: String(record.id_vaca), fecha: date ? date.toISOString().slice(0, 16) : '', causa: String(record.causa ?? ''), meses_gestacion: String(record.meses_gestacion ?? ''), descripcion: String(record.descripcion ?? '') }); }} onDelete={(id) => setDeleting({ type: 'abortion', id })} /> : null}
