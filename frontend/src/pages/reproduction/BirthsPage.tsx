...
const emptyAbortion = (): AbortionForm => ({ id_vaca: '', fecha: localToday(), causa: '', meses_gestacion: '', descripcion: '' });
...
export function BirthsPage() {
  ...
  const heatList = useListControls({ items: heats.data ?? [], storageKey: 'reproduction-heats', searchText: (item) => `${item.vaca} ${item.codigo_arete ?? ''} ${item.toro ?? ''}`, dateValue: (item) => item.fecha_inicio, nameValue: (item) => item.vaca });
  const pregnancyList = useListControls({ items: pregnancies.data ?? [], storageKey: 'reproduction-pregnancies', searchText: (item) => `${item.vaca} ${item.codigo_arete ?? ''} ${item.padre ?? ''} ${item.metodo_confirmacion}`, dateValue: (item) => item.fecha_confirmacion, nameValue: (item) => item.vaca });
  const upcomingList = useListControls({ items: upcoming.data ?? [], storageKey: 'reproduction-upcoming', searchText: (item) => `${item.vaca} ${item.codigo_arete ?? ''} ${item.padre ?? ''}`, dateValue: (item) => item.fecha_tentativa ?? item.fecha_confirmacion, nameValue: (item) => item.vaca });
  const birthList = useListControls({ items: births.data ?? [], storageKey: 'reproduction-births', searchText: (item) => `${item.madre} ${item.madre_arete ?? ''} ${item.padre ?? ''} ${item.tipo_parto} ${item.crias.map((child) => `${child.cria} ${child.codigo_arete ?? ''}`).join(' ')}`, dateValue: (item) => item.fecha_parto, nameValue: (item) => item.madre });
  const abortionList = useListControls({ items: abortions.data ?? [], storageKey: 'reproduction-abortions', searchText: (item) => `${String(item.animal ?? '')} ${String(item.causa ?? '')} ${String(item.descripcion ?? '')}`, dateValue: (item) => String(item.fecha ?? ''), nameValue: (item) => String(item.animal ?? '') });
  ...
  const saveAbortion = useMutation({
    mutationFn: () => {
      ...
      const body = { id_vaca: abortionForm.id_vaca, fecha: abortionForm.fecha || null, causa: nullIfEmpty(abortionForm.causa), meses_gestacion: numberOrNull(abortionForm.meses_gestacion), descripcion: nullIfEmpty(abortionForm.descripcion) };
  ...
  return <div>
    ...
    {tab === 'abortions' ? <AbortionsList query={abortions} items={abortionList.visible} canEdit={hasPermission('ABORTO_ADMINISTRAR')} onEdit={(record) => { setAbortionForm({ id_aborto: String(record.id_aborto), id_vaca: String(record.id_vaca), fecha: dateInputValue(String(record.fecha ?? '')), causa: String(record.causa ?? ''), meses_gestacion: String(record.meses_gestacion ?? ''), descripcion: String(record.descripcion ?? '') }); }} onDelete={(id) => setDeleting({ type: 'abortion', id })} /> : null}

    {selectedBirth ? <BirthDetail birth={selectedBirth} onClose={() => setSelectedBirth(null)} onEdit={hasPermission('PARTO_ADMINISTRAR') ? () => editBirth(selectedBirth) : undefined} /> : null}
