...
export function BirthsPage() {
  ...
  const routeYearStart=searchParams.get('periodo')==='anio'?`${currentDateInput().slice(0,4)}-01-01`:'';
  const birthList = useListControls({ items: (births.data ?? []).filter((item) => (!routeYearStart||dateInputValue(item.fecha_parto)>=routeYearStart)&&inScope(item.categoria_codigo,[item.id_madre,item.id_padre,...item.crias.map(child=>child.id_cria)])), storageKey: 'reproduction-births', searchText: (item) => `${item.madre} ${item.madre_arete ?? ''} ${item.padre ?? ''} ${item.tipo_parto} ${item.crias.map((child) => `${child.cria} ${child.codigo_arete ?? ''}`).join(' ')}`, dateValue: (item) => item.fecha_parto, nameValue: (item) => item.madre });
  ...
  return <div className="module-no-header">
    ...
    </div>{routeYearStart&&tab==='births'?<span className="active-route-filter">Este año<button type="button" aria-label="Quitar filtro de período" onClick={()=>{const next=new URLSearchParams(searchParams);next.delete('periodo');setSearchParams(next,{replace:true});}}><X size={14}/></button></span>:null}</>}/>
