...
import type { Birth, GenericRecord, Group, HeatRecord, PregnancyRecord, UpcomingBirth } from '../../types/api';
...
type ReproductionAnimal = { id_animal: string; nombre: string; codigo_arete: string | null; fecha_nacimiento: string | null; id_especie: string; id_categoria_animal: string; id_propiedad: string | null; categoria_codigo: string; categoria: string };
...
export function BirthsPage() {
  ...
  return <div>
    ...
    {birthForm ? <BirthModal form={birthForm} setForm={setBirthForm} pregnancies={pendingPregnancies} species={species.data ?? []} origins={origins.data ?? []} groups={groups.data ?? []} saving={createBirth.isPending} onSave={() => createBirth.mutate()} onClose={() => setBirthForm(null)} /> : null}
...
function BirthModal({ form, setForm, pregnancies, species, origins, groups, saving, onSave, onClose }: { form: BirthForm; setForm: (form: BirthForm) => void; pregnancies: PregnancyRecord[]; species: Record<string, unknown>[]; origins: Record<string, unknown>[]; groups: Group[]; saving: boolean; onSave: () => void; onClose: () => void }) {
