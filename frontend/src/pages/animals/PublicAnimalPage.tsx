...
import { Beef, CalendarDays, Download, FileImage, FileText, HeartPulse, Images, Maximize2, Scale, Tag, Users } from 'lucide-react';
...
export function PublicAnimalPage(){
  const {token=''}=useParams();const [downloading,setDownloading]=useState<'png'|'pdf'|null>(null);const [selectedPhoto,setSelectedPhoto]=useState<PublicAnimal['fotos_portada'][number]|null>(null);
  ...
  const animal=query.data!;const coverPhotos=animal.fotos_portada??[];const breeds=animal.razas.map((item)=>`${item.nombre}${item.porcentaje!=null?` ${formatNumber(item.porcentaje)}%`:''}`).join(', ');
  return <main className="public-animal-page">
    ...
    <Card className="public-animal-card">
      ...
      {coverPhotos.length?<section className="public-animal-gallery"><header><div><h2><Images size={21}/>Fotos de portada</h2><p>Fotografías compartidas de {animal.nombre}.</p></div><Badge tone="neutral">{coverPhotos.length}</Badge></header><div>{coverPhotos.map((photo)=><button type="button" key={photo.id_imagen} onClick={()=>setSelectedPhoto(photo)}><img src={photo.secure_url} alt={photo.descripcion||`Foto de ${animal.nombre}`}/><span><small>{photo.fecha_toma?formatDate(photo.fecha_toma):'Sin fecha'}</small>{photo.descripcion?<strong>{photo.descripcion}</strong>:null}</span><Maximize2 size={18}/></button>)}</div></section>:null}
