...
export function PublicAnimalPage(){
  ...
  const viewerItems=[
    ...(animal.foto_perfil?[{key:'profile',url:animal.foto_perfil,type:'IMAGEN' as const,title:`${animal.nombre} · Foto de perfil`,subtitle:'Foto de perfil'}]:[]),
    ...coverPhotos.filter((photo)=>photo.secure_url!==animal.foto_perfil).map((photo)=>({key:`cover-${photo.id_imagen}`,url:photo.secure_url,type:'IMAGEN' as const,title:`${animal.nombre} · Foto de portada`,subtitle:photo.descripcion,date:photo.fecha_toma})),
  ];
  const coverViewerIndex=(photo:PublicAnimal['fotos_portada'][number])=>Math.max(0,viewerItems.findIndex((item)=>item.key===`cover-${photo.id_imagen}`));
  return <main className="public-animal-page">
    ...
    <Card className="public-animal-card">
      <section className="public-animal-hero">
        {currentCover?<button type="button" className="public-animal-cover" onClick={()=>setViewerIndex(coverViewerIndex(currentCover))}><img src={currentCover.secure_url} alt={`Portada de ${animal.nombre}`}/><Maximize2 size={20}/></button>:<div className="public-animal-cover public-animal-cover-empty"><Beef size={72}/></div>}
        ...
        <div className="public-animal-identity"><button type="button" className="public-animal-avatar" disabled={!animal.foto_perfil} aria-label={`Abrir foto de perfil de ${animal.nombre}`} onClick={()=>setViewerIndex(0)}>{animal.foto_perfil?<><img src={animal.foto_perfil} alt={animal.nombre}/><Maximize2 size={18}/></>:<Beef size={52}/>}</button><div><Badge tone={animal.estado==='ACTIVO'?'success':'neutral'}>{humanizeCode(animal.estado)}</Badge><h1>{animal.nombre}</h1><p>{animal.codigo_arete?`Arete ${animal.codigo_arete}`:'Sin arete registrado'}</p></div></div>
      </section>
      ...
      {coverPhotos.length?<section className="public-animal-gallery"><header><div><h2><Images size={21}/>Fotos de portada</h2><p>Fotografías compartidas de {animal.nombre}.</p></div><Badge tone="neutral">{coverPhotos.length}</Badge></header><div>{coverPhotos.map((photo)=><button type="button" key={photo.id_imagen} onClick={()=>setViewerIndex(coverViewerIndex(photo))}><img src={photo.secure_url} alt={photo.descripcion||`Foto de ${animal.nombre}`}/><span><small>{photo.fecha_toma?formatDate(photo.fecha_toma):'Sin fecha'}</small>{photo.descripcion?<strong>{photo.descripcion}</strong>:null}</span><Maximize2 size={18}/></button>)}</div></section>:null}
    ...
    {viewerIndex!==null?<ImageLightbox items={viewerItems} initialIndex={viewerIndex} onClose={()=>setViewerIndex(null)}/>:null}
