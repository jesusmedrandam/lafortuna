...
import { Beef, ChevronDown, ChevronLeft, ChevronRight, MapPin, Plus, RotateCcw, SlidersHorizontal, UserRound, Weight, X } from 'lucide-react';
import { useNavigate, useSearchParams } from 'react-router-dom';
import { apiRequest, apiRequestWithMeta } from '../../api/client';
import { useAuth } from '../../auth/AuthContext';
import { Badge, Button, EmptyState, ErrorState, Field, Input, LoadingState, PageHeader, SearchBox, Select } from '../../components/ui';
import { useDebouncedValue } from '../../hooks/useDebouncedValue';
import type { Animal, AnimalFilterOptions } from '../../types/api';
import { formatNumber, humanizeCode } from '../../utils';
import { AnimalFormModal } from './AnimalFormModal';

const pageSize = 20;
const filterKeys = ['sexo', 'estado', 'id_especie', 'id_grupo', 'id_ubicacion', 'id_propietario', 'id_raza', 'id_color', 'nacimiento_desde', 'nacimiento_hasta'] as const;
const advancedFilterKeys = ['id_especie', 'id_grupo', 'id_ubicacion', 'id_propietario', 'id_raza', 'id_color', 'nacimiento_desde', 'nacimiento_hasta'] as const;
  ...
  return <div>
    ...
    <div className="toolbar animal-search-toolbar">
      ...
      <div className="toolbar-filters">
        ...
        <Button type="button" variant={advancedOpen ? 'secondary' : 'ghost'} onClick={() => setAdvancedOpen((current) => !current)} aria-expanded={advancedOpen}><SlidersHorizontal size={18} />Más filtros{activeFilterCount ? <span className="filter-count">{activeFilterCount}</span> : null}<ChevronDown className={advancedOpen ? 'chevron-open' : ''} size={16} /></Button>
      </div>
    </div>

    {advancedOpen ? <section className="advanced-filters" aria-label="Búsqueda avanzada de animales">
      <div className="advanced-filters-heading"><div><h2>Búsqueda avanzada</h2><p>Combina varios criterios para encontrar animales específicos.</p></div><Button type="button" variant="ghost" onClick={() => setAdvancedOpen(false)}><X size={17} />Cerrar</Button></div>
      ...
      <div className="advanced-filters-footer"><span>{activeFilterCount ? `${activeFilterCount} filtro${activeFilterCount === 1 ? '' : 's'} activo${activeFilterCount === 1 ? '' : 's'}` : 'Sin filtros aplicados'}</span><Button type="button" variant="ghost" disabled={!activeFilterCount} onClick={clearFilters}><RotateCcw size={16} />Limpiar filtros</Button></div>
    </section> : null}
