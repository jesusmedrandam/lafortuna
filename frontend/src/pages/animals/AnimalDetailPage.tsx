...
interface AnimalShare { activo:boolean;token:string;url:string;created_at:string;pendiente_sincronizacion?:boolean }
...
export function AnimalDetailPage() {
  ...
  const productionCanRegister=availability.data?.produccion?.registrar===true;
  const productionCanConsult=availability.data?.produccion?.consultar===true;
  ...
  const canHeat=hasPermission('PARTO_ADMINISTRAR')&&availability.data?.celo?.permitido===true;
  const canInseminate=hasPermission('PARTO_ADMINISTRAR')&&availability.data?.inseminacion?.permitido===true;
  const canImplantEmbryo=hasPermission('PARTO_ADMINISTRAR')&&availability.data?.embrion?.permitido===true;
  const canConfirmPregnancy=hasPermission('PARTO_ADMINISTRAR')&&availability.data?.prenez?.permitido===true;
  ...
  return <div className="animal-detail-page">
    ...
    <div className="animal-profile-action-strip" aria-label="Acciones del animal">
      ...
      {hasPermission('MOVIMIENTO_CREAR')&&availability.data?.movimiento?.permitido===true?<ProfileAction icon={ArrowRightLeft} label="Movimiento" onClick={()=>openIfAllowed(availability.data?.movimiento,()=>setQuickAction('movement'))}/>:null}
      {hasPermission('SANIDAD_ADMINISTRAR')&&availability.data?.sanidad?.permitido===true?<ProfileAction icon={Syringe} label="Sanidad" onClick={()=>openIfAllowed(availability.data?.sanidad,()=>setQuickAction('health'))}/>:null}
      {hasPermission('PESAJE_ADMINISTRAR')&&availability.data?.pesaje?.permitido===true?<ProfileAction icon={Weight} label="Pesaje" onClick={()=>openIfAllowed(availability.data?.pesaje,()=>navigate(`/pesajes?animal=${animal.id_animal}&nuevo=1`))}/>:null}
      {productionActionVisible?<ProfileAction icon={Milk} label="Producción" onClick={()=>setQuickAction('production')}/>:null}
      {reproductionActionVisible?<ProfileAction icon={Baby} label="Reproducción" onClick={()=>setQuickAction('reproduction')}/>:null}
      {hasPermission('VENTA_CREAR')&&availability.data?.venta?.permitido===true?<ProfileAction icon={ShoppingCart} label="Vender" onClick={()=>openIfAllowed(availability.data?.venta,()=>navigate(`/ventas?animal=${animal.id_animal}&nuevo=1`))}/>:null}
      {hasPermission('MUERTE_CREAR')&&availability.data?.muerte?.permitido===true?<ProfileAction icon={HeartOff} label="Muerte" danger onClick={()=>openIfAllowed(availability.data?.muerte,()=>navigate(`/muertes?animal=${animal.id_animal}&nuevo=1`))}/>:null}
    ...
    {quickAction==='production'?<Modal title={`Producción de ${animal.nombre}`} onClose={()=>setQuickAction(null)}><div className="profile-quick-action-list">{hasProductionHistory&&productionCanConsult&&hasPermission('PRODUCCION_CONSULTAR')?<button type="button" onClick={()=>navigate(`/produccion?animal=${animal.id_animal}`)}><History size={21}/><span><strong>Ver producción</strong><small>Consultar únicamente los registros y lactancias de {animal.nombre}.</small></span><ChevronRight size={18}/></button>:null}{productionCanRegister&&hasPermission('PRODUCCION_ADMINISTRAR')?<button type="button" onClick={()=>navigate(`/produccion?animal=${animal.id_animal}&nuevo=1`)}><Milk size={21}/><span><strong>Registrar producción</strong><small>{animal.nombre} quedará seleccionada automáticamente.</small></span><ChevronRight size={18}/></button>:null}</div></Modal>:null}
...
function ProfileAction({icon:Icon,label,onClick,danger=false}:{icon:LucideIcon;label:string;onClick:()=>void;danger?:boolean}){
  return <button type="button" aria-label={label} title={label} className={danger?'profile-action danger':'profile-action'} onClick={onClick}><Icon size={21}/></button>;
}
