...
import {
  ...
  Trash2, UserCog, Users, WalletCards, Weight,
  type LucideIcon,
} from 'lucide-react';
...
type SettingsSection = 'inicio' | 'notificaciones' | 'apariencia' | 'finca' | 'operaciones' | 'enlaces' | 'actualizacion'|'agenda'|'hora'|'resumen'|'finanzas';
...
interface PersonalFinanceConfiguration{habilitadas:boolean;permitir_saldo_negativo:boolean;moneda:string;updated_at?:string|null}
...
function sectionFrom(value: string | null): SettingsSection {
  return value === 'notificaciones' || value === 'apariencia' || value === 'finca' || value === 'operaciones' || value === 'enlaces' || value === 'actualizacion'||value==='agenda'||value==='hora'||value==='resumen'||value==='finanzas' ? value : 'inicio';
}
...
export function SettingsPage() {
  ...
  const financeQuery=useQuery({queryKey:['personal-finance-configuration'],queryFn:()=>apiRequest<PersonalFinanceConfiguration>('/mis-finanzas/configuracion'),enabled:section==='finanzas'});const[financeDraft,setFinanceDraft]=useState<PersonalFinanceConfiguration>({habilitadas:false,permitir_saldo_negativo:false,moneda:'USD'});useEffect(()=>{if(financeQuery.data)setFinanceDraft(financeQuery.data);},[financeQuery.data]);const saveFinance=useMutation({mutationFn:()=>apiRequest<PersonalFinanceConfiguration>('/mis-finanzas/configuracion',{method:'PUT',body:{habilitadas:financeDraft.habilitadas,permitir_saldo_negativo:financeDraft.permitir_saldo_negativo}}),onSuccess:async(data)=>{setFinanceDraft(data);toast.show('Configuración financiera guardada.');await client.invalidateQueries({queryKey:['personal-finance-configuration']});},onError:error=>toast.show((error as ApiError).message,'error')});
  ...
  if(section==='finanzas')return <div className="settings-page settings-content">{sectionHeader('Mis finanzas','Contabilidad personal y privada para tu usuario.',<Button loading={saveFinance.isPending} onClick={()=>saveFinance.mutate()}><Save size={18}/>Guardar</Button>)}{financeQuery.isLoading?<LoadingState/>:financeQuery.isError?<ErrorState message={(financeQuery.error as Error).message} onRetry={()=>void financeQuery.refetch()}/>:<Card className="farm-settings-form"><label className="settings-switch-row"><span><strong>Activar Mis finanzas</strong><small>Muestra el módulo únicamente en tu cuenta. Ningún otro usuario puede consultar tus datos.</small></span><span className="switch"><input type="checkbox" checked={financeDraft.habilitadas} onChange={event=>setFinanceDraft(current=>({...current,habilitadas:event.target.checked}))}/><i/></span></label><label className={`settings-switch-row ${!financeDraft.habilitadas?'disabled':''}`}><span><strong>Permitir saldos negativos</strong><small>Si está desactivado, se impedirá gastar más dinero del disponible en la cuenta.</small></span><span className="switch"><input type="checkbox" disabled={!financeDraft.habilitadas} checked={financeDraft.permitir_saldo_negativo} onChange={event=>setFinanceDraft(current=>({...current,permitir_saldo_negativo:event.target.checked}))}/><i/></span></label><div className="settings-info"><WalletCards size={19}/><span>Solo se guardan nombres generales como “Efectivo” o “Pichincha 1”. No se solicitan números de cuenta, tarjetas ni claves.</span></div>{financeDraft.habilitadas?<Button variant="secondary" onClick={()=>navigate('/mis-finanzas')}><WalletCards size={18}/>Abrir Mis finanzas</Button>:null}</Card>}</div>;
  ...
  const personalCards: SettingsCardDefinition[] = [
    ...
    { label: 'Mis finanzas', description: 'Activa tu contabilidad personal y privada.', icon: WalletCards, action: () => openSection('finanzas') },
  ];
