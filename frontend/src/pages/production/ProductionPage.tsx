...
import type { Animal, Birth, GenericRecord, Property } from '../../types/api';
...
export function ProductionPage() {
  ...
  const properties = useQuery({ queryKey: ['properties', 'production'], queryFn: () => apiRequest<Property[]>('/propiedades') });
  const principalProperty = properties.data?.find((item) => item.es_principal && item.activa);
  const females = useQuery({ queryKey: ['animals', 'production-females', principalProperty?.id_propiedad], queryFn: () => apiRequest<Animal[]>(`/animales?limit=100&sexo=HEMBRA&id_propiedad=${principalProperty!.id_propiedad}`), enabled: Boolean(principalProperty) });
  ...
  return <div>
    ...
    <div className="form-alert"><Milk size={18} /><span>{principalProperty ? <>El ordeño está habilitado únicamente para animales de <strong>{principalProperty.nombre}</strong>.</> : 'Selecciona una propiedad principal antes de registrar lactancias u ordeños.'}</span></div>
