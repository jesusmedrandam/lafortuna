...
async function latestAppRelease() {
    ...
    if (response.status === 404)
        return null;
    if (!response.ok)
        throw new Error(`GitHub no respondió correctamente (HTTP ${response.status}).`);
    const release = await response.json();
    const version = String(release.tag_name ?? '').replace(/^v/i, '').trim();
    if (!/^\d+(?:\.\d+)+$/.test(version))
        return null;
    const apk = (release.assets ?? []).find(asset => String(asset.name ?? '').toLocaleLowerCase('es').endsWith('.apk')
...
async function notifyOverdueCleanings(client, today) {
    const rows = (await client.query(`SELECT p.id_potrero,u.nombre,MAX(COALESCE(l.fecha_finalizacion,l.fecha_inicio))::text ultima_limpieza,
       CASE WHEN MAX(COALESCE(l.fecha_finalizacion,l.fecha_inicio)) IS NULL THEN NULL
         ELSE ($1::date-MAX(COALESCE(l.fecha_finalizacion,l.fecha_inicio))::date)::int END dias
     FROM potrero p
     ...
     HAVING MAX(COALESCE(l.fecha_finalizacion,l.fecha_inicio)) IS NULL
       OR $1::date-MAX(COALESCE(l.fecha_finalizacion,l.fecha_inicio))::date >= $2`, [today, env.CLEANING_ALERT_DAYS])).rows;
