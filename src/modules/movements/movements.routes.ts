...
async function validateMovementSelection(database: Queryable, input: MovementInput): Promise<MovementValidation> {
  ...
  const location = (await database.query(
    `SELECT u.tipo,u.id_categoria_animal,ca.codigo categoria_codigo
    ...
    [effectiveLocationId],
  )).rows[0] as { tipo: string; id_categoria_animal: string; categoria_codigo: string } | undefined;
  ...
  const currentAnimals = (await database.query(
    `SELECT id_animal,id_categoria_animal,id_grupo_actual,id_ubicacion_actual
     FROM animal WHERE id_animal=ANY($1::uuid[]) AND deleted_at IS NULL AND estado='ACTIVO' FOR SHARE`,
    [animalIds],
  )).rows as Array<{ id_animal: string; id_categoria_animal: string; id_grupo_actual: string | null; id_ubicacion_actual: string | null }>;
