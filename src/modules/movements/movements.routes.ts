...
async function validateMovementSelection(database: Queryable, input: MovementInput) {
  ...
  const location = input.id_ubicacion_destino
    ? (await database.query('SELECT tipo,id_categoria_animal FROM ubicacion WHERE id_ubicacion=$1 AND deleted_at IS NULL', [input.id_ubicacion_destino])).rows[0] as { tipo: string; id_categoria_animal: string } | undefined
    : undefined;
  const group = input.id_grupo_destino
    ? (await database.query('SELECT id_categoria_animal FROM grupo WHERE id_grupo=$1 AND deleted_at IS NULL AND activo=TRUE', [input.id_grupo_destino])).rows[0] as { id_categoria_animal: string } | undefined
    : undefined;

  if (input.id_ubicacion_destino && !location) throw new ValidationError('La ubicación de destino no está disponible.');
  if (input.id_grupo_destino && !group) throw new ValidationError('El grupo de destino no está disponible.');
  ...
  for (const animal of selected) {
    ...
    if (input.tipo_movimiento === 'GRUPO' && group) {
      const current = (await database.query('SELECT id_categoria_animal FROM animal WHERE id_animal=$1', [animal.id_animal])).rows[0] as { id_categoria_animal: string };
...
movementsRouter.get('/', requirePermission('MOVIMIENTO_CONSULTAR'), asyncHandler(async (_req, res) => ok(res, (await pool.query(
  `SELECT m.*,u1.nombre ubicacion_origen,u2.nombre ubicacion_destino,g1.nombre grupo_origen,g2.nombre grupo_destino,
...
movementsRouter.post('/', requirePermission('MOVIMIENTO_CREAR'), asyncHandler(async (req, res) => {
  ...
  const result = await transaction(async (client) => {
    ...
    for (const animal of animales) await client.query(buildInsert('movimiento_animal_detalle', { ...animal, id_movimiento: row.id_movimiento }));
    return row;
  }, req.user!.id);
...
movementsRouter.patch('/:id', requirePermission('MOVIMIENTO_CREAR'), asyncHandler(async (req, res) => {
  ...
  const row = await transaction(async (client) => {
    ...
    return (await client.query(
      `UPDATE movimiento_animal SET fecha_movimiento=$2,motivo=$3,observaciones=$4,updated_at=NOW()
       WHERE id_movimiento=$1 RETURNING *`,
      [id, input.fecha_movimiento ?? found.fecha_movimiento, input.motivo === undefined ? found.motivo : input.motivo, input.observaciones === undefined ? found.observaciones : input.observaciones],
    )).rows[0];
  }, req.user!.id);
  return ok(res, row);
}));
...
movementsRouter.put('/:id/seleccion', requirePermission('MOVIMIENTO_CREAR'), asyncHandler(async (req, res) => {
  ...
  await transaction(async (client) => {
    ...
    for (const animal of items) await client.query(buildInsert('movimiento_animal_detalle', { ...animal, id_movimiento: id }));
