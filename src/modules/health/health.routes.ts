...
async function assertReferences(database:Queryable,animalId:string,typeId?:string|null) {
  ...
  if(!animal.rowCount)throw new ValidationError('El animal no está activo o disponible.');
  if(typeId) {
    ...
    if(!type.rowCount)throw new ValidationError('El tipo de condición no está disponible.');
  }
}

export const healthRouter=Router();

healthRouter.get('/',requirePermission('SANIDAD_CONSULTAR'),asyncHandler(async(_req,res)=>ok(res,(await pool.query(
  `SELECT c.*,a.nombre animal,a.codigo_arete,ca.codigo categoria_codigo,ca.nombre categoria,t.nombre tipo_condicion,
   ...
   ORDER BY CASE c.estado WHEN 'POR_RESOLVER' THEN 1 WHEN 'EN_TRATAMIENTO' THEN 2 ELSE 3 END,c.fecha_deteccion DESC`
)).rows)));
...
healthRouter.patch('/:id',requirePermission('SANIDAD_ADMINISTRAR'),asyncHandler(async(req,res)=>{
  ...
  const row=await transaction(async client=>{
    ...
    const current=(await client.query(
      `SELECT c.id_animal,c.estado,EXISTS(
         SELECT 1 FROM tratamiento_animal t
         WHERE t.id_condicion_salud=c.id_condicion_salud AND t.deleted_at IS NULL
       ) tiene_tratamientos
       FROM condicion_salud c WHERE c.id_condicion_salud=$1 AND c.deleted_at IS NULL FOR UPDATE`,[id],
    )).rows[0] as {id_animal:string;estado:string;tiene_tratamientos:boolean}|undefined;
    if(!current||current.estado==='RESUELTA')throw new NotFoundError('Condición no encontrada o ya resuelta.');
    ...
    return (await client.query(
      `UPDATE condicion_salud SET id_animal=$2,id_tipo_condicion_salud=$3,fecha_deteccion=$4,descripcion=$5,updated_at=NOW()
       WHERE id_condicion_salud=$1 RETURNING *`,
      [id,input.id_animal,input.id_tipo_condicion_salud??null,input.fecha_deteccion,input.descripcion],
    )).rows[0];
  },req.user!.id);
  return ok(res,row);
}));
...
healthRouter.delete('/:id',requirePermission('SANIDAD_ADMINISTRAR'),asyncHandler(async(req,res)=>{
  const result=await pool.query(
    `UPDATE condicion_salud SET deleted_at=NOW(),updated_at=NOW()
  ...
  if(!result.rowCount)throw new ValidationError('No se puede eliminar una condición que ya tiene tratamientos relacionados.');
