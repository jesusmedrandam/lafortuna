...
const schema = z.object({
  ...
  if(!input.id_tipo_limpieza&&!input.id_tipos_limpieza.length)ctx.addIssue({code:z.ZodIssueCode.custom,path:['id_tipos_limpieza'],message:'Selecciona al menos una actividad de limpieza.'});
});

type CleaningInput = z.infer<typeof schema>;

function cleaningTypeIds(input:CleaningInput) {
  return [...new Set(input.id_tipos_limpieza.length?input.id_tipos_limpieza:(input.id_tipo_limpieza?[input.id_tipo_limpieza]:[]))];
}

async function replaceCleaningTypes(client:Parameters<Parameters<typeof transaction>[0]>[0],cleaningId:string,ids:string[],userId:string) {
  const valid=await client.query(
    'SELECT id_tipo_limpieza FROM tipo_limpieza_potrero WHERE id_tipo_limpieza=ANY($1::uuid[]) AND activo=TRUE AND deleted_at IS NULL',[ids],
  );
  if(valid.rowCount!==ids.length)throw new ValidationError('Una o más actividades de limpieza no están disponibles.');
  ...
  for(const id of ids)await client.query(buildInsert('limpieza_potrero_actividad',{id_limpieza:cleaningId,id_tipo_limpieza:id,registrado_por:userId}));
}
...
function calculatedProducts(input: CleaningInput) {
...
cleaningsRouter.patch('/:id', requirePermission('LIMPIEZA_ADMINISTRAR'), asyncHandler(async (req, res) => {
  ...
  const result = await transaction(async (client) => {
    const found = (await client.query(
      'SELECT id_limpieza FROM limpieza_potrero WHERE id_limpieza=$1 AND deleted_at IS NULL FOR UPDATE',
      [id],
    )).rows[0];
    if (!found) throw new NotFoundError('Limpieza no encontrada.');
    const { productos: _products, operadores, id_tipos_limpieza:_types, id_tipo_limpieza:_legacyType, ...head } = input;
    const types=cleaningTypeIds(input);
    const row = (await client.query(
      `UPDATE limpieza_potrero SET id_potrero=$2,id_tipo_limpieza=$3,fecha_inicio=$4,fecha_finalizacion=$5,
       ...
       WHERE id_limpieza=$1 RETURNING *`,
      [id,head.id_potrero,types[0],head.fecha_inicio,head.fecha_finalizacion ?? null,
        head.unidad_aplicacion,head.cantidad_tanques ?? null,head.capacidad_tanque_litros ?? null,
        head.tipo_area_intervenida,head.estado,head.observaciones ?? null],
    )).rows[0];
