...
settingsRouter.put('/operaciones-animales', requirePermission('CATALOGO_ADMINISTRAR'), asyncHandler(async (req, res) => {
  ...
  await transaction(async (client) => {
    for (const item of input.configuracion) {
      await client.query(
        `INSERT INTO operacion_propiedad_animal(id_propiedad,codigo_operacion,permitido,registrado_por)
         VALUES($1,$2,$3,$4)
        ...
        [item.id_propiedad, item.codigo_operacion, item.permitido, req.user!.id],
      );
    }
  }, req.user!.id);
