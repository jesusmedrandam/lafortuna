...
async function assertPropertyChangeAllowed(database: Queryable,idLocation:string,nextProperty?:string){
  if(!nextProperty)return;
  const current=(await database.query('SELECT id_propiedad FROM ubicacion WHERE id_ubicacion=$1 AND deleted_at IS NULL',[idLocation])).rows[0] as {id_propiedad:string}|undefined;
  if(!current)throw new NotFoundError('Ubicación no encontrada.');
  if(current.id_propiedad===nextProperty)return;
...
pasturesRouter.patch('/:id',requirePermission('POTRERO_ADMINISTRAR'),asyncHandler(async(req,res)=>{const input=pastureSchema.partial().parse(req.body);const result=await transaction(async c=>{const id=routeParam(req.params.id,'id');const found=await c.query('SELECT id_ubicacion FROM potrero WHERE id_potrero=$1 AND deleted_at IS NULL',[id]);if(!found.rows[0])throw new NotFoundError();if(input.ubicacion){await assertPropertyChangeAllowed(c,found.rows[0].id_ubicacion,input.ubicacion.id_propiedad);await c.query(buildUpdate('ubicacion','id_ubicacion',found.rows[0].id_ubicacion,input.ubicacion));}const {ubicacion,pastos,...rest}=input;if(Object.keys(rest).length)await c.query(buildUpdate('potrero','id_potrero',id,rest));if(pastos){await c.query('UPDATE potrero_pasto SET deleted_at=NOW() WHERE id_potrero=$1 AND deleted_at IS NULL',[id]);for(const item of pastos)await c.query(buildInsert('potrero_pasto',{...item,id_potrero:id}));}return {id_potrero:id};},req.user!.id);return ok(res,result);}));
...
corralsRouter.patch('/:id',requirePermission('CORRAL_ADMINISTRAR'),asyncHandler(async(req,res)=>{const input=corralSchema.partial().parse(req.body);const result=await transaction(async c=>{const id=routeParam(req.params.id,'id');const found=await c.query('SELECT id_ubicacion FROM corral WHERE id_corral=$1 AND deleted_at IS NULL',[id]);if(!found.rows[0])throw new NotFoundError();if(input.ubicacion){await assertPropertyChangeAllowed(c,found.rows[0].id_ubicacion,input.ubicacion.id_propiedad);await c.query(buildUpdate('ubicacion','id_ubicacion',found.rows[0].id_ubicacion,input.ubicacion));}const {ubicacion,...rest}=input;if(Object.keys(rest).length)await c.query(buildUpdate('corral','id_corral',id,rest));return {id_corral:id};},req.user!.id);return ok(res,result);}));
