...
import { buildInsert, buildUpdate, type Queryable } from '../shared/sql.js';
  ...
  if (input.id_propiedad_padre) {
    const property=(await database.query(
      `SELECT id_categoria_animal FROM ubicacion
       WHERE id_ubicacion=$1 AND tipo='OTRO' AND activo=TRUE AND deleted_at IS NULL`,
      [input.id_propiedad_padre],
    )).rows[0] as { id_categoria_animal: string } | undefined;
    if (!property) throw new ValidationError('La propiedad externa seleccionada no está disponible.');
    return {...input,id_categoria_animal:property.id_categoria_animal};
  }
  const owned=(await database.query(
    `SELECT id_categoria_animal FROM categoria_animal
     WHERE codigo='EN_PROPIEDAD' AND activo=TRUE AND deleted_at IS NULL LIMIT 1`,
  )).rows[0] as { id_categoria_animal: string } | undefined;
  if (!owned) throw new ValidationError('No está configurada la categoría de animales en propiedad.');
...
locationsRouter.delete('/:id',requirePermission('UBICACION_ADMINISTRAR'),asyncHandler(async(req,res)=>{const id=routeParam(req.params.id,'id');const usage=(await pool.query(`SELECT EXISTS(SELECT 1 FROM ubicacion WHERE id_propiedad_padre=$1 AND deleted_at IS NULL) has_locations,EXISTS(SELECT 1 FROM grupo WHERE id_ubicacion_actual=$1 AND activo=TRUE AND deleted_at IS NULL) has_groups,EXISTS(SELECT 1 FROM animal WHERE id_ubicacion_actual=$1 AND estado='ACTIVO' AND deleted_at IS NULL) has_animals`,[id])).rows[0];if(usage.has_locations||usage.has_groups||usage.has_animals)throw new ConflictError('No puede desactivar una propiedad que tenga potreros, corrales, grupos o animales activos.');const r=await pool.query('UPDATE ubicacion SET deleted_at=NOW(),activo=FALSE WHERE id_ubicacion=$1 AND deleted_at IS NULL',[id]);if(!r.rowCount)throw new NotFoundError();return noContent(res);}));
...
pasturesRouter.patch('/:id',requirePermission('POTRERO_ADMINISTRAR'),asyncHandler(async(req,res)=>{const input=pastureSchema.partial().parse(req.body);const result=await transaction(async c=>{const found=await c.query('SELECT p.id_ubicacion,u.id_propiedad_padre FROM potrero p JOIN ubicacion u ON u.id_ubicacion=p.id_ubicacion WHERE p.id_potrero=$1 AND p.deleted_at IS NULL',[routeParam(req.params.id, 'id')]);if(!found.rows[0])throw new NotFoundError();if(input.ubicacion){await ensurePropertyCanChange(c,found.rows[0].id_ubicacion,found.rows[0].id_propiedad_padre,input.ubicacion.id_propiedad_padre);const location=await productiveLocation(c,input.ubicacion);await c.query(buildUpdate('ubicacion','id_ubicacion',found.rows[0].id_ubicacion,location));}const {ubicacion,pastos,...rest}=input;if(Object.keys(rest).length)await c.query(buildUpdate('potrero','id_potrero',routeParam(req.params.id, 'id'),rest));if(pastos){await c.query('UPDATE potrero_pasto SET deleted_at=NOW() WHERE id_potrero=$1 AND deleted_at IS NULL',[routeParam(req.params.id, 'id')]);for(const item of pastos)await c.query(buildInsert('potrero_pasto',{...item,id_potrero:routeParam(req.params.id, 'id')}));}return {id_potrero:routeParam(req.params.id, 'id')};},req.user!.id);return ok(res,result);}));
...
corralsRouter.patch('/:id',requirePermission('CORRAL_ADMINISTRAR'),asyncHandler(async(req,res)=>{const input=corralSchema.partial().parse(req.body);const result=await transaction(async c=>{const found=await c.query('SELECT c.id_ubicacion,u.id_propiedad_padre FROM corral c JOIN ubicacion u ON u.id_ubicacion=c.id_ubicacion WHERE c.id_corral=$1 AND c.deleted_at IS NULL',[routeParam(req.params.id, 'id')]);if(!found.rows[0])throw new NotFoundError();if(input.ubicacion){await ensurePropertyCanChange(c,found.rows[0].id_ubicacion,found.rows[0].id_propiedad_padre,input.ubicacion.id_propiedad_padre);const location=await productiveLocation(c,input.ubicacion);await c.query(buildUpdate('ubicacion','id_ubicacion',found.rows[0].id_ubicacion,location));}const {ubicacion,...rest}=input;if(Object.keys(rest).length)await c.query(buildUpdate('corral','id_corral',routeParam(req.params.id, 'id'),rest));return {id_corral:routeParam(req.params.id, 'id')};},req.user!.id);return ok(res,result);}));
