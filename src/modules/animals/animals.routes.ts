...
animalsRouter.patch('/:id', requirePermission('ANIMAL_MODIFICAR'), asyncHandler(async (req, res) => {
  ...
  const result = await transaction(async (client) => {
    ...
    const current = (await client.query(
      'SELECT id_especie,id_madre,id_padre,id_categoria_animal,id_ubicacion_actual,estado FROM animal WHERE id_animal=$1 AND deleted_at IS NULL FOR UPDATE',
      [id],
    )).rows[0] as { id_especie: string; id_madre: string | null; id_padre: string | null; id_categoria_animal: string; id_ubicacion_actual: string | null; estado: string } | undefined;
    ...
    if (animal.estado !== undefined) await assertAnimalCondition(client, animal.estado, animal.estado === current.estado);
    let row;
