...
animalsRouter.get('/', requirePermission('ANIMAL_CONSULTAR'), asyncHandler(async (req, res) => {
  ...
  if (p.categoria_codigo) add('ca.codigo=?', p.categoria_codigo);
  if (p.clasificacion) add('fn_clasificacion_animal(a.id_animal,CURRENT_DATE)=?', p.clasificacion);
  if (p.propiedad_principal) where.push('pg.es_principal=TRUE');
...
animalsRouter.get('/enlaces-publicos', requirePermission('ANIMAL_MODIFICAR'), asyncHandler(async (req, res) => {
  ...
  const rows = (await pool.query(
    `SELECT ac.id_animal_compartido,ac.id_animal,a.nombre animal,a.codigo_arete,
  ...
  )).rows.map((row) => ({ ...row, url: sharedAnimalUrl(row.token) }));
  return ok(res, rows);
}));

animalsRouter.delete('/enlaces-publicos/:shareId', requirePermission('ANIMAL_MODIFICAR'), asyncHandler(async (req, res) => {
  if (!req.user!.roles.includes('ADMINISTRADOR')) throw new ForbiddenError('Solo un administrador puede desactivar enlaces creados por otros usuarios.');
  const result = await pool.query(
    `UPDATE animal_compartido SET activo=FALSE,revocado_at=NOW(),updated_at=NOW()
     WHERE id_animal_compartido=$1 AND activo=TRUE AND revocado_at IS NULL`,
    [z.string().uuid().parse(routeParam(req.params.shareId, 'shareId'))],
  );
  if (!result.rowCount) throw new NotFoundError('El enlace público ya no está activo.');
...
animalsRouter.get('/:id/ubicacion-historica', requirePermission('ANIMAL_CONSULTAR'), asyncHandler(async (req, res) => {
  ...
  const [animalResult, locationResult, groupResult, snapshotResult] = await Promise.all([
    pool.query(
      `SELECT a.id_animal,a.nombre,a.created_at,a.created_at::date::text fecha_registro,a.id_ubicacion_actual,a.id_grupo_actual
  ...
  if(fecha<animal.fecha_registro)return ok(res,{
