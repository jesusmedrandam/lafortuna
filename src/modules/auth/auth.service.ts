...
async function loadUser(client: PoolClient | typeof pool, id: string): Promise<AuthenticatedUser | null> {
...
export async function getAuthenticatedUser(id: string): Promise<AuthenticatedUser | null> {
  return cache.remember('usuarios', `auth:${id}`, 60, () => loadUser(pool, id));
}

async function upsertChallenge(client: PoolClient, userId: string, purpose: 'ACTIVACION'|'RESTABLECER_PASSWORD', code: string) {
...
async function issueTokens(client: PoolClient, user: AuthenticatedUser, req: { ip?: string; userAgent?: string }) {
...
export async function register(input: { nombres:string; apellidos:string; correo:string; password:string; telefono?:string }) {
  const email = normalize(input.correo); const code = randomCode();
  const user = await transaction(async (client) => {
    const exists = await client.query('SELECT 1 FROM usuario WHERE LOWER(correo)=$1 AND deleted_at IS NULL', [email]);
    if (exists.rowCount) throw new ConflictError('Ya existe una cuenta con ese correo.');
...
async function verifyChallenge(client: PoolClient, email: string, purpose: 'ACTIVACION'|'RESTABLECER_PASSWORD', code: string) {
  ...
  const row = result.rows[0];
  if (!row) throw new AppError(400, 'Código inválido o expirado.', 'INVALID_CODE');
  if (row.bloqueado_hasta && new Date(row.bloqueado_hasta) > new Date()) throw new AppError(429, 'Demasiados intentos. Intenta más tarde.', 'CODE_LOCKED');
  if (row.consumido_en || new Date(row.expira_en) < new Date()) throw new AppError(400, 'Código inválido o expirado.', 'INVALID_CODE');
...
export async function resendVerification(email:string) {
  const normalized=normalize(email); const code=randomCode();
  ...
  if (!user) return { message:'Si la cuenta existe, se enviará un código.' };
  if (user.correo_verificado) throw new ConflictError('La cuenta ya está verificada.');
...
export async function login(email:string,password:string,request:{ip?:string;userAgent?:string}) {
  ...
  return transaction(async client=>{
    ...
    if (ctrl?.bloqueado_hasta && new Date(ctrl.bloqueado_hasta)>new Date()) throw new AppError(429,'Cuenta temporalmente bloqueada por intentos fallidos.','LOGIN_LOCKED');
    ...
    if (!row.correo_verificado) throw new AppError(403,'Debes verificar tu correo.','EMAIL_NOT_VERIFIED');
    if (!row.activo) throw new AppError(403,'La cuenta está desactivada.','ACCOUNT_DISABLED');
    ...
    const user=await loadUser(client,row.id_usuario); if(!user) throw new UnauthorizedError();
    return issueTokens(client,user,request);
  });
}

export async function refresh(refreshToken:string, request:{ip?:string;userAgent?:string}) {
  let payload; try { payload=verifyRefreshToken(refreshToken); } catch { throw new UnauthorizedError('El token de renovación no es válido.'); }
  return transaction(async client=>{
    const stored=await client.query('SELECT * FROM refresh_token WHERE id_refresh_token=$1 FOR UPDATE',[payload.jti]);
    const row=stored.rows[0];
    if(!row || row.revocado_en || new Date(row.expira_en)<new Date() || row.token_hash!==tokenHash(refreshToken)) throw new UnauthorizedError('La sesión ya no es válida.');
    const user=await loadUser(client,payload.sub);
    if(!user || user.sessionVersion!==payload.sv) throw new UnauthorizedError('La sesión fue invalidada.');
...
export async function logout(refreshToken:string) {
  try { const payload=verifyRefreshToken(refreshToken); await pool.query('UPDATE refresh_token SET revocado_en=COALESCE(revocado_en,NOW()) WHERE id_refresh_token=$1',[payload.jti]); } catch { /* respuesta idempotente */ }
...
export async function forgotPassword(email:string) {
  ...
  const user=result.rows[0];
  if(user){ await transaction(c=>upsertChallenge(c,user.id_usuario,'RESTABLECER_PASSWORD',code)); await sendCodeEmail(normalized,code,'RESTABLECER_PASSWORD'); }
...
export async function changePassword(userId:string,currentPassword:string,newPassword:string) {
  return transaction(async client=>{
    const user=(await client.query(
      'SELECT password_hash FROM usuario WHERE id_usuario=$1 AND deleted_at IS NULL FOR UPDATE',
      [userId],
    )).rows[0];
    ...
    await client.query(
      'UPDATE usuario SET password_hash=$2,password_changed_at=NOW(),version_sesion=version_sesion+1,updated_at=NOW() WHERE id_usuario=$1',
      [userId,hash],
    );
    await client.query(
      'UPDATE refresh_token SET revocado_en=COALESCE(revocado_en,NOW()) WHERE id_usuario=$1',
      [userId],
    );
    cache.forgetModuleVersion('usuarios');
...
export async function updateProfile(
  ...
  let verification: { correo: string; codigo: string } | null = null;

  const updated = await transaction(async (client) => {
    const currentResult = await client.query(
      'SELECT * FROM usuario WHERE id_usuario=$1 AND deleted_at IS NULL FOR UPDATE',
      [userId],
    );
    ...
    if (input.nombres !== undefined) add('nombres', input.nombres);
    if (input.apellidos !== undefined) add('apellidos', input.apellidos);
    if (input.telefono !== undefined) add('telefono', input.telefono);
    if (input.fecha_nacimiento !== undefined) add('fecha_nacimiento', input.fecha_nacimiento);

    if (input.correo !== undefined) {
      const nextEmail = normalize(input.correo);
      if (nextEmail !== normalize(current.correo)) {
        const exists = await client.query(
          'SELECT 1 FROM usuario WHERE LOWER(correo)=$1 AND id_usuario<>$2 AND deleted_at IS NULL',
          [nextEmail, userId],
        );
        if (exists.rowCount) throw new ConflictError('Ya existe una cuenta con ese correo.');
    ...
    const result = await client.query(
      `UPDATE usuario SET ${sets.join(',')} WHERE id_usuario=$1 AND deleted_at IS NULL
       RETURNING id_usuario,nombres,apellidos,telefono,correo,fecha_nacimiento,foto_perfil_url,correo_verificado,activo`,
      [userId, ...values],
    );
  ...
  cache.forgetModuleVersion('usuarios');
  const pendingVerification = verification as { correo: string; codigo: string } | null;
...
export async function updateUserAsAdmin(
  ...
  const updated = await transaction(async (client) => {
    ...
    const current = currentResult.rows[0];
    if (!current) throw new AppError(404, 'Usuario no encontrado.', 'NOT_FOUND');
    ...
    if (!emailChanged && input.activo !== undefined) add('activo', input.activo);
    add('version_sesion', Number(current.version_sesion) + 1);
    sets.push('updated_at=NOW()', 'version=version+1');
