...
salesRouter.patch('/productos/:id', requirePermission('VENTA_ADMINISTRAR'), asyncHandler(async (req, res) => {
  ...
  const sale = await transaction(async (client) => {
    const current = (await client.query(
      `SELECT estado FROM venta_producto WHERE id_venta_producto=$1 AND deleted_at IS NULL FOR UPDATE`, [id],
    )).rows[0];
    if (!current) throw new NotFoundError('Venta de productos no encontrada.');
    if (current.estado === 'ANULADA') throw new ConflictError('Una venta anulada no se puede modificar.');
    const buyer = (await client.query(
      `SELECT id_comprador,nombre,contacto,destino FROM comprador WHERE id_comprador=$1 AND deleted_at IS NULL AND activo=TRUE`,
      [input.id_comprador],
    )).rows[0];
    if (!buyer) throw new NotFoundError('El comprador no existe o está inactivo.');
    const products = (await client.query(
...
salesRouter.patch('/:id', requirePermission('VENTA_ADMINISTRAR'), asyncHandler(async (req, res) => {
  ...
  const row = await transaction(async (client) => {
    const current = (await client.query(
      `SELECT estado FROM venta_animal WHERE id_venta=$1 AND deleted_at IS NULL FOR UPDATE`, [id],
    )).rows[0];
    if (!current) throw new NotFoundError('Venta no encontrada.');
    if (current.estado === 'ANULADA') throw new ConflictError('Una venta anulada no se puede modificar.');
    const buyer = (await client.query(
      `SELECT id_comprador,nombre,contacto,destino FROM comprador WHERE id_comprador=$1 AND deleted_at IS NULL AND activo=TRUE`,
      [input.id_comprador],
    ...
    return (await client.query(
      `UPDATE venta_animal SET fecha_venta=$2,id_comprador=$3,comprador_nombre=$4,comprador_contacto=$5,
