...
selectionRouter.post('/animales/preview', requirePermission('ANIMAL_CONSULTAR'), asyncHandler(async (req, res) => {
  ...
  if (input.modo === 'GRUPO') {
    if (!input.id_grupo) throw new ValidationError('El grupo es obligatorio.');
  ...
  if (input.filtros.id_especie) add('a.id_especie', input.filtros.id_especie);
  if (input.filtros.id_propiedad) add('COALESCE(g.id_propiedad,u.id_propiedad)', input.filtros.id_propiedad);
  if (input.filtros.sexo) add('a.sexo', input.filtros.sexo);
  if (input.filtros.id_ubicacion) add('a.id_ubicacion_actual', input.filtros.id_ubicacion);
  ...
  if (input.filtros.situacion_propiedad === 'EN_PROPIEDAD') where.push("ca.codigo='EN_PROPIEDAD'");
  if (input.filtros.situacion_propiedad === 'FUERA_PROPIEDAD') where.push("ca.codigo<>'EN_PROPIEDAD'");
  if (input.filtros.estado) add('a.estado', input.filtros.estado);
  ...
  const rows = (await pool.query(
    `SELECT a.id_animal,a.codigo_arete,a.nombre,a.sexo,a.id_categoria_animal,
     ...
     WHERE ${where.join(' AND ')} ORDER BY a.nombre`,
    params,
  )).rows;
