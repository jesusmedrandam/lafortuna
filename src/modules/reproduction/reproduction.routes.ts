...
async function activeAnimal(client: PoolClient, id: string, sex: 'HEMBRA' | 'MACHO', role: string) {
  const animal = (await client.query(
    `SELECT id_animal,id_especie,nombre,sexo,fecha_nacimiento,estado
     FROM animal WHERE id_animal=$1 AND deleted_at IS NULL FOR SHARE`,
    [id],
...
async function actionAvailability(check:()=>Promise<unknown>):Promise<Availability>{
  try{
    ...
    if(error instanceof AppError)return{permitido:false,motivo:error.message};
    throw error;
  }
}
...
reproductionRouter.get('/disponibilidad/:id',requirePermission('ANIMAL_CONSULTAR'),asyncHandler(async(req,res)=>{
  ...
  )).rows[0] as {id_prenez:string}|undefined;
  const validFemale=animal.estado==='ACTIVO'&&animal.sexo==='HEMBRA';
  const unavailable=(message:string):Availability=>({permitido:false,motivo:message});
  const femaleCheck=()=>{
    if(!validFemale)throw new ValidationError('Esta operación solo está disponible para hembras activas.');
  };
  ...
  const production=await actionAvailability(async()=>{
    ...
    const recentBirth=(await pool.query(
      `SELECT 1 FROM parto
       ...
       LIMIT 1`,[animalId,date,rules.dias_maximos_ordeno_posparto],
    )).rowCount;
    if(!recentBirth)throw new ValidationError(`No tiene un parto dentro de los ${rules.dias_maximos_ordeno_posparto} días permitidos para ordeño.`);
  });
  const unavailableActions:[Availability,Availability,Availability,Availability,Availability,Availability,Availability]=[
...
reproductionRouter.get('/celos', requirePermission('PARTO_CONSULTAR'), asyncHandler(async (_req, res) => ok(res, (await pool.query(
  `SELECT c.*,v.nombre vaca,v.codigo_arete,t.nombre toro,t.codigo_arete toro_arete,
   ...
   WHERE c.deleted_at IS NULL ORDER BY c.fecha_inicio DESC,c.created_at DESC`,
)).rows)));
...
reproductionRouter.delete('/celos/:id', requirePermission('PARTO_ADMINISTRAR'), asyncHandler(async (req, res) => {
  ...
  await transaction(async (client) => {
    const linked = await client.query('SELECT 1 FROM prenez WHERE id_celo=$1 AND deleted_at IS NULL LIMIT 1', [id]);
    if (linked.rowCount) throw new ValidationError('No se puede eliminar un celo que ya tiene una preñez relacionada.');
    const result = await client.query('UPDATE celo SET deleted_at=NOW() WHERE id_celo=$1 AND deleted_at IS NULL', [id]);
    if (!result.rowCount) throw new NotFoundError('Celo no encontrado.');
  }, req.user!.id);
...
reproductionRouter.post('/preneces', requirePermission('PARTO_ADMINISTRAR'), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if ((error as { code?: string }).code === '23505') throw new ValidationError('Esta vaca ya tiene una preñez confirmada pendiente de finalizar.');
    throw error;
  }
}));
...
reproductionRouter.patch('/preneces/:id', requirePermission('PARTO_ADMINISTRAR'), asyncHandler(async (req, res) => {
  ...
  const row = await transaction(async (client) => {
    ...
    await client.query(
      ...
      [id, data.id_vaca, data.fecha_parto_tentativa],
    );
    return pregnancy;
  }, req.user!.id);
  return ok(res, row);
...
reproductionRouter.delete('/preneces/:id', requirePermission('PARTO_ADMINISTRAR'), asyncHandler(async (req, res) => {
  ...
  await transaction(async (client) => {
    const result = await client.query(
      `UPDATE prenez SET estado='CANCELADA',updated_at=NOW()
       WHERE id_prenez=$1 AND deleted_at IS NULL AND estado='CONFIRMADA'`,
      [id],
    );
    if (!result.rowCount) throw new NotFoundError('Preñez confirmada no encontrada.');
...
reproductionRouter.get('/proximos-partos', requirePermission('PARTO_CONSULTAR'), asyncHandler(async (_req, res) => ok(res, (await pool.query(
  `SELECT pp.*,p.fecha_confirmacion,p.metodo_embarazo,p.metodo_confirmacion,p.dias_gestacion_confirmacion,
   ...
   ORDER BY pp.fecha_tentativa NULLS LAST,v.nombre`,
)).rows)));
