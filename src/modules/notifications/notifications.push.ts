...
function versionIsNewer(candidate: string, current: string | null) {
...
async function sendRecipient(item: PendingRecipient) {
  try {
    ...
    const devices = (await pool.query<{token_push:string;version_app:string|null;plataforma:string}>(`
