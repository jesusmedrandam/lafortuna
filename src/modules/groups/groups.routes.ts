...
groupsRouter.patch('/:id',requirePermission('GRUPO_ADMINISTRAR'),asyncHandler(async(req,res)=>{const data=schema.partial().parse(req.body);const row=(await pool.query(buildUpdate('grupo','id_grupo',routeParam(req.params.id, 'id'),pick(data,columns)))).rows[0];if(!row)throw new NotFoundError();return ok(res,row);}));
groupsRouter.delete('/:id',requirePermission('GRUPO_ADMINISTRAR'),asyncHandler(async(req,res)=>{const r=await pool.query('UPDATE grupo SET deleted_at=NOW(),activo=FALSE WHERE id_grupo=$1 AND deleted_at IS NULL',[routeParam(req.params.id, 'id')]);if(!r.rowCount)throw new NotFoundError();return noContent(res);}));
