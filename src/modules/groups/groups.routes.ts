...
const columns = ['codigo', 'nombre', 'id_tipo_grupo', 'id_categoria_animal', 'id_propiedad', 'id_ubicacion_actual', 'id_especie', 'descripcion', 'capacidad', 'activo'] as const;

async function validateGroupLocation(database: Queryable, locationId?: string | null) {
  ...
  const location = (await database.query(
    ...
    [locationId],
  )).rows[0] as { id_propiedad: string; id_categoria_animal: string } | undefined;
...
async function completeGroupCode(database: Queryable, code: string | null | undefined, name: string, propertyId: string, excludeId?: string) {
  ...
  for (let attempt = 1; attempt <= 999; attempt += 1) {
    ...
    const existing = await database.query(
      `SELECT 1 FROM grupo
       ...
       LIMIT 1`,
      [candidate, propertyId, excludeId ?? null],
    );
...
groupsRouter.patch('/:id', requirePermission('GRUPO_ADMINISTRAR'), asyncHandler(async (req, res) => {
  ...
  const current = (await pool.query(
    `SELECT id_categoria_animal,id_propiedad,id_ubicacion_actual FROM grupo
     WHERE id_grupo=$1 AND deleted_at IS NULL`,
    [id],
  )).rows[0] as { id_categoria_animal: string; id_propiedad: string; id_ubicacion_actual: string | null } | undefined;
  ...
  if (nextLocation !== current.id_ubicacion_actual) {
    ...
    if (members.rowCount) throw new ConflictError('Un grupo con animales debe cambiar de potrero, corral o propiedad mediante un movimiento de grupo completo.');
  }
  if (placement.id_categoria_animal!==current.id_categoria_animal) {
    const incompatible = await pool.query(
      `SELECT 1 FROM animal
       ...
       LIMIT 1`,
      [id,placement.id_categoria_animal],
    );
...
groupsRouter.delete('/:id', requirePermission('GRUPO_ADMINISTRAR'), asyncHandler(async (req, res) => {
  ...
  await transaction(async client=>{
    const group=await client.query(
      'SELECT id_grupo FROM grupo WHERE id_grupo=$1 AND deleted_at IS NULL FOR UPDATE',
      [id],
    );
    if(!group.rowCount)throw new NotFoundError();
    const members=await client.query(
      `SELECT 1 FROM animal
       WHERE id_grupo_actual=$1 AND estado='ACTIVO' AND deleted_at IS NULL LIMIT 1`,
      [id],
    );
    if(members.rowCount)throw new ConflictError('No puede eliminar un grupo que todavía contiene animales activos. Trasládelos primero.');
