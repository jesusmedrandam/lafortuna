...
usersRouter.get('/', requirePermission('USUARIO_CONSULTAR'), asyncHandler(async (_req, res) => ok(res, (
  await pool.query(`
...
usersRouter.put('/:id/roles', requirePermission('USUARIO_ADMINISTRAR'), asyncHandler(async (req, res) => {
  ...
  await transaction(async (client) => {
    ...
    if (!exists.rowCount) throw new NotFoundError('Usuario no encontrado.');

    if (roles.length) {
      const validRoles = await client.query(
        'SELECT id_rol FROM rol WHERE id_rol=ANY($1::uuid[]) AND deleted_at IS NULL AND activo',
        [roles],
      );
      if (validRoles.rowCount !== roles.length) throw new ConflictError('Uno de los roles no existe o está inactivo.');
    }

    await client.query(
      'UPDATE usuario_rol SET deleted_at=NOW(),updated_at=NOW(),version=version+1 WHERE id_usuario=$1 AND deleted_at IS NULL',
      [userId],
    );
    for (const roleId of roles) {
      await client.query(
        'INSERT INTO usuario_rol(id_usuario,id_rol,asignado_por) VALUES($1,$2,$3)',
        [userId, roleId, req.user!.id],
      );
    }
    await client.query(
      'UPDATE usuario SET version_sesion=version_sesion+1,updated_at=NOW(),version=version+1 WHERE id_usuario=$1',
      [userId],
    );
    await client.query(
...
usersRouter.delete('/:id', requirePermission('USUARIO_ADMINISTRAR'), asyncHandler(async (req, res) => {
  const userId = routeParam(req.params.id, 'id');
  if (userId === req.user!.id) throw new ConflictError('No puedes eliminar tu propia cuenta desde administración.');
  ...
  if (!result.rowCount) throw new NotFoundError('Usuario no encontrado.');
  cache.forgetModuleVersion('usuarios');
...
rolesRouter.get('/', requirePermission('ROL_CONSULTAR'), asyncHandler(async (_req, res) => ok(res, (
  await pool.query(`
    SELECT r.*,
           COALESCE(
...
rolesRouter.get('/permisos', requirePermission('ROL_CONSULTAR'), asyncHandler(async (_req, res) => ok(res, (
  await pool.query('SELECT * FROM permiso WHERE deleted_at IS NULL AND activo ORDER BY modulo,nombre')
).rows)));
...
rolesRouter.patch('/:id', requirePermission('ROL_ADMINISTRAR'), asyncHandler(async (req, res) => {
  ...
  if (!current.rows[0]) throw new NotFoundError('Rol no encontrado.');
  const data = z.object({
...
rolesRouter.put('/:id/permisos', requirePermission('ROL_ADMINISTRAR'), asyncHandler(async (req, res) => {
  ...
  await transaction(async (client) => {
    ...
    if (!role.rows[0]) throw new NotFoundError('Rol no encontrado.');
...
rolesRouter.delete('/:id', requirePermission('ROL_ADMINISTRAR'), asyncHandler(async (req, res) => {
  ...
  if (!role.rows[0]) throw new NotFoundError('Rol no encontrado.');
  if (role.rows[0].protegido) throw new ConflictError('Este rol está protegido y no puede eliminarse.');
  await transaction(async (client) => {
