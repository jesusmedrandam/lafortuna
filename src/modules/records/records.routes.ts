...
async function validateLactation(client:TransactionClient,input:LactationInput,excludeId?:string) {
  ...
  const birth=(await client.query(
    `SELECT id_madre,fecha_parto::date::text AS fecha_parto,
       (fecha_parto::date + $2::int)::text AS fecha_limite
     FROM parto
     WHERE id_parto=$1 AND deleted_at IS NULL FOR SHARE`,[input.id_parto,rules.dias_maximos_ordeno_posparto],
  ...
  if(input.fecha_fin&&input.fecha_fin>birth.fecha_limite)throw new ValidationError(`La lactancia no puede extenderse más de ${rules.dias_maximos_ordeno_posparto} días después del parto.`);
  if(input.activa&&new Date().toISOString().slice(0,10)>birth.fecha_limite)throw new ValidationError(`Esta lactancia superó los ${rules.dias_maximos_ordeno_posparto} días configurados desde el parto y debe registrarse como finalizada.`);
...
recordsRouter.get('/producciones/vacas-activas',requirePermission('PRODUCCION_CONSULTAR'),asyncHandler(async(req,res)=>{
  ...
  return ok(res,(await pool.query(
    `WITH vacas AS MATERIALIZED (
     ...
     ORDER BY v.nombre,v.codigo_arete`,[date,turn,excludeId??null],
  )).rows);
}));
...
recordsRouter.get('/producciones/vacas-elegibles',requirePermission('PRODUCCION_CONSULTAR'),asyncHandler(async(req,res)=>{
  ...
  return ok(res,(await pool.query(
    `WITH vacas AS MATERIALIZED (
     ...
     FROM vacas v JOIN partos p ON p.id_madre=v.id_animal
     LEFT JOIN lactancias l ON l.id_vaca=v.id_animal
     ORDER BY v.nombre,v.codigo_arete`,[date],
  )).rows);
}));
    ...
    if(input.en_ordeno) {
      const recentBirth=(await client.query(
        `SELECT id_parto FROM parto
         ...
         ORDER BY fecha_parto DESC LIMIT 1 FOR SHARE`,[id,referenceDate,rules.dias_maximos_ordeno_posparto],
      )).rows[0];
      if(!recentBirth)throw new ValidationError(`Para iniciar el ordeño la vaca debe tener un parto dentro de los últimos ${rules.dias_maximos_ordeno_posparto} días.`);
    }
...
recordsRouter.patch('/:module/:id', asyncHandler(async (req, res) => {
  ...
  if(d.table==='lactancia') {
    ...
    const row=await transaction(async client=>{
      const current=(await client.query(
        'SELECT id_lactancia,id_vaca,en_ordeno,activa FROM lactancia WHERE id_lactancia=$1 AND deleted_at IS NULL FOR UPDATE',[id],
      )).rows[0] as {id_lactancia:string;id_vaca:string;en_ordeno:boolean;activa:boolean}|undefined;
      ...
      if(current.activa&&saved.activa===false)await notifyRecordCreated(client,'lactancias',saved,req.user!.id);
      return saved;
    },req.user!.id);
    return ok(res,row);
  }
  ...
  if(d.table==='tratamiento_animal') {
    ...
    const row=await transaction(async client=>{
      ...
      await assertMedicationApplication(
        client,data.id_medicamento??current.id_medicamento,
        data.id_via_administracion??current.id_via_administracion,
        data.id_unidad_dosis??current.id_unidad_dosis,
        data.id_tipo_tratamiento??current.id_tipo_tratamiento,
      );
